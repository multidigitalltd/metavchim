...
-- ‎**וברירת המחדל נשארת.**
--
-- ‏היא אינה רק לרגע ההוספה: העמודה **נגזרת**, והמשמעות
-- ‏הטבעית של היעדרה היא „אין שכונות” — לעולם לא „שורה פסולה”.
-- ‏בלעדיה כל יצירת קונה שאינה מצרפת את העמודה נכשלת ב-NOT NULL,
-- ‏וזה מה שקרה בפועל: שער `cross-tenant` נפל על שורות בדיקה
-- ‏שיוצרות קונים ישירות (ביקורת Codex). את הסיכון של שכחה
-- ‏במסלול כתיבה תופס השער שב-`requirement-columns.test.ts`, שדורש
-- ‏שכל הכתיבות יעברו דרך `requirementColumns`.
--
-- ‏בסכימה היא מוצהרת כ-`@default([])`, כדי שלא תיווצר סחיפה.
ALTER TABLE "buyers"
