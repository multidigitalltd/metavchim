...
-- ‏1. ‎**דירוגים אנונימיים שכבר קיימים יורדים.** אין להם `user_id`
-- ‏   ואין דרך לשחזר ממנו זהות (`rater_key` הוא חתם HMAC חד-כיווני),
-- ‏   כלומר שורה שאינה יכולה לקיים את הכלל. המונים של הרשומה
-- ‏   מתוקנים באותה פקודה — אחרת הממוצע היה נשאר על ספירה שכבר אינה.
WITH removed AS (
