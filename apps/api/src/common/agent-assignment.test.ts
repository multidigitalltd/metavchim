...
describe("שיוך סוכן — נכס, ליד וקונה", () => {
  ...
  /*
   * ‎**שורה אחת — ומותר יותר משדה אחד ממנה.**
   *
   * ‏כרטיס הנכס שולף שני שמות מאותה שורה (הסוכן המטפל והסוכן
   * ‏השותף), וזו עדיין **שאילתה אחת**: מה שהשער מונע הוא N+1 ו„רק
   * ‏הראשון מקבל שם”, ושניהם נמדדים במספר השאילתות ולא במספר
   * ‏השדות. התנאי `\1` מקבע שכל המזהים באים מאותה שורה — שתי
   * ‏שורות שונות באותו מערך היו כן לולאה שהתחפשה.
   */
  const SINGLE = /^\[(row|r)\.\w+(?:,\s*\1\.\w+)*,?\]$/u;
  ...
  it.each([
    ...
    /* ‏ארגומנט פרוס על שורות הוא אותו ארגומנט — הרווחים אינם המידע */
    const ids = agentNamesArgs(API(path))
      .map(idsArg)
      .map((arg) => arg.replace(/\s+/gu, " ").replace(/\[\s+/u, "[").replace(/,?\s*\]/u, "]"));
