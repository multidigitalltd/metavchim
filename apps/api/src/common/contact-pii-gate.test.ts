...
const CLASSIFIED: Record<string, Entry> = {
  ...
  /*
   * ‎**המנטור — הקונים כבר מסוננים בבעלות, ב-SQL הגולמי.**
   *
   * ‏„העסקה הקרובה ביותר” מפענחת שם אחד: של הלקוח שמאחורי כרטיס
   * ‏הקונה שנבחר. ארבע השאילתות שמזינות את הרשימה נושאות
   * ‏`b.owner_user_id = ${userId}`, ולכן מועמד שאינו של המתווך אינו
   * ‏מגיע לפענוח מלכתחילה — וההתראה שנוצרת היא **אישית**
   * ‏(`userId: user.id`), לא משרדית.
   *
   * ‏מוצהר `entity` עם `gate` מקומי ולא `system`, אף שהסבב רץ בלי
   * ‏משתמש בהקשר: יש כאן היקף אמיתי לכל מתווך, והוא **ניתן
   * ‏לאימות**. הצהרת `system` הייתה קביעה אנושית שאיש לא יבדוק
   * ‏שוב, ואילו הסמל שלמטה נופל כאן ברגע שמישהו יסיר את הסינון.
   *
   * ‏ו-`gate` לבדו אינו מספיק כאן, כי הוא מוכיח ש**קיים** סינון
   * ‏אחד ולא שכולם מסננים: מוטציה שהסירה אחד מחמשת התנאים עברה
   * ‏בשקט. הספירה למטה סוגרת את זה.
   */
  "modules/mentor/mentor-signals.service.ts": {
    as: "entity",
    why: "שם הלקוח של הקונה שלי — כל השאילתות מסננות `owner_user_id`",
    gate: /owner_user_id = \$\{userId\}/u,
  },
...
/**
 * ‎**„כל השאילתות מסננות” — נספר, לא מוצהר.**
 *
 * ‏ההצהרה של `mentor-signals.service.ts` היא שכל חיבור לטבלת
 * ‏הקונים נושא את `owner_user_id`. `gate` בודק שהביטוי **קיים**
 * ‏בקובץ, וזה נכשל בדיוק במקרה המעניין: הסרת אחד מחמשת התנאים
 * ‏משאירה ארבעה, והביטוי עדיין נמצא.
 *
 * ‏שאילתה אחת בלי הסינון היא רשימת קונים של כל המשרד — ומשם השם
 * ‏המפוענח של לקוח של עמית.
 */
