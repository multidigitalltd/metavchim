...
const RLS_EXEMPT: Readonly<Record<string, string>> = {
  ...
  // מסמכי המס נגזרים מאותן שורות סליקה, והמטאטא שמפיק אותם סורק את
  // התור של כל המשרדים יחד — בלי משתמש ובלי הקשר דייר. קריאה של
  // משרד עוברת דרך `downloadUrl(id, tenantId)`, שמסננת במפורש.
  invoices: "חשבוניות מס — מופקות במטאטא חוצה-דיירים, כמו payments",
