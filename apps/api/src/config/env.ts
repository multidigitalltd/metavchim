...
const EnvSchema = z.object({
   ...
   * ל-Coexistence אצל Meta; מחרוזת ריקה מחזירה Embedded Signup רגיל.
  ...
  WHATSAPP_SIGNUP_FEATURE_TYPE: z
    .enum(["", "whatsapp_business_app_onboarding"])
    .optional(),
   ...
   *
   * הפרדה ולא שימוש חוזר: תשובות של לקוחות למשרדים ופניות תמיכה הן
   * שני זרמים עם כללי זיהוי שונים לגמרי (טוקן חובה מול פנייה
   * ראשונה בלי טוקן), ושרת נפרד אצל הספק פירושו גם כתובת נפרדת וגם
   * שתקלה באחד אינה נוגעת בשני.
   */
  SUPPORT_INBOUND_ADDRESS: z.string().email().optional(),
  SUPPORT_INBOUND_SECRET: z.string().min(16).optional(),
  /**
