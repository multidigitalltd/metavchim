...
describe("שער: הכלל מנוסח במקום אחד", () => {
  ...
  /*
   * ‎**והחלון שאחרי שהספק אישר אינו שייך לסיווג הזה בכלל.**
   *
   * ‏אחרי ש-`email.send` חזר בהצלחה, ההודעה **אצל הלקוח**. הכתיבה
   * ‏שמאשרת „נשלח” היא תיעוד בלבד, וחריגה שלה שיוצאת מהפונקציה
   * ‏מגיעה אל אותו `catch` שמסווג שגיאות שליחה — ומדווחת „נכשלה”
   * ‏על מייל שהגיע. הסוכן שולח שוב, מזהה חדש פירושו מפתח ייחודיות
   * ‏חדש, והלקוח מקבל עותק שני (ביקורת Codex, P1).
   *
   * ‏זהו אותו נימוק בדיוק שכתוב ב-`EmailService.settle` — „חריגה
   * ‏כאן הייתה הופכת שליחה מוצלחת לכישלון מדווח” — ושלושה נתיבים
   * ‏כבר מיישמים אותו, שניים מהם עם אותו משפט מילה במילה. הרביעי
   * ‏שכח, וזו הפעם הרביעית בסבב הזה שכלל שנוסח במקום אחד נשכח
   * ‏בנתיב שני. לכן הוא נאכף, ולא רק מתועד.
   */
  it("‏כתיבת האישור „נשלח” לעולם אינה יכולה לזרוק החוצה", () => {
    const offenders: string[] = [];
    for (const { name, src } of FILES) {
      let from = 0;
      for (;;) {
        const at = src.indexOf('sendState: "sent"', from);
        if (at === -1) break;
        from = at + 1;
        /* ‏תחילת המשפט החיצוני: כל ארבעת הנתיבים כותבים דרך `this.prisma`. */
        const start = src.lastIndexOf("await this.prisma", at);
        if (start === -1) {
          offenders.push(`${name}: כתיבה שאינה דרך this.prisma`);
          continue;
        }
        let depth = 0;
        let end = start;
        ...
        if (!src.slice(start, end).includes(".catch(")) offenders.push(name);
      }
    }
    expect(offenders, "אישור שיכול להפוך שליחה שהצליחה לכישלון").toEqual([]);
  });
