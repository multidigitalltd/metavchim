...
export type PlatformSettingKey =
  ...
  /**
   * ‎**האם התבנית שנרשמה נושאת שורה לכל עדכון, או פירוט אחד.**
   *
   * ריק/לא מסומן = פירוט אחד (`update_details`) — החוזה שלפיו
   * נרשמו התבניות הקיימות. ‏Meta אינה מתירה ירידת שורה **בערך**
   * של תבנית, ולכן הפירוט משוטח ל-`·`, ותקציר המנטור — שנכתב
   * שורה לכל נושא — הגיע כשרשרת אחת ארוכה (דיווח מהשטח). שורות
   * אמיתיות אפשריות רק בגוף התבנית, כלומר במשתנה לכל שורה, וזו
   * תבנית **אחרת** שצריך לרשום ב-WhatsApp Manager.
   *
   * ברירת המחדל היא הישנה מאותו נימוק כמו `...ReminderTemplateFields`:
   * שליחת חמישה שמות לתבנית שיש בה שניים נדחית אצל Meta, וההתראה
   * נעלמת בלי שאיש יידע.
   */
