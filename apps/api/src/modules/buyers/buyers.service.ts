...
export class BuyersService {
  ...
  async purge(id: string): Promise<void> {
    ...
    await this.prisma.withTenant(async (tx) => {
       ...
       * שהכרטיס נמחק, ונשאר במסד עם שם, טלפונים ואימייל. אותו מצב
       * בדיוק כמו במחיקת נכס לצמיתות, ואותה הכרעה: `eraseUnreachable`
       * בודק את אותו מבחן יתמות ומחזיר `false` על כרטיס שעדיין נגיש
       * — קונה אחר, ליד, נכס, או היותו בן/בת זוג בכרטיס חי.
       *
