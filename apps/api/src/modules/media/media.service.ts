...
export class MediaService {
   ...
   * ההזמנה: היא כבר נרשמה. `notifiedAt` נכתב רק כשהמייל לנציג יצא
   * (או כשאין נציג ומנהלי הפלטפורמה קיבלו במקומו), כדי שהמסך של
   * הפלטפורמה יראה מה טרם נשלח.
   */
  private async notify(orderId: string): Promise<void> {
     ...
     * כתובת, זה גם הערוץ היחיד שההזמנה מגיעה בו, ולכן הוא נספר
     * כמסירה.
     */
    const admins = await this.admins.notify({
    ...
    if (contactEmail === "" && admins.sent > 0) delivered = true;
