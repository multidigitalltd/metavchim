...
describe("buttonAsText", () => {
  ...
  /*
   * ‎**הכפתור שנשאר בהיסטוריה אחרי שהמנוי חודש.**
   *
   * ‏„חידוש המנוי” נשלח למשרד שתקופתו נגמרה ומטופל שם לפני
   * ‏המנוע — אבל אחרי התשלום המשרד כבר אינו „נגמר”, ולחיצה על
   * ‏הכפתור הישן מגיעה לכאן. `null` היה מגלגל אותה ל„אני יודע
   * ‏לטפל כרגע בטקסט ובהודעות קוליות” — תשובה חסרת פשר על כפתור
   * ‏שהמערכת עצמה שלחה.
   */
  it("„חידוש המנוי” ישן הופך לבקשת חידוש, ולא לתשובה חסרת פשר", () => {
    expect(buttonAsText("renew")).toBe("תחדש את המנוי");
  });
