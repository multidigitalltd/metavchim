...
export function helpMenu(allowedIds: readonly string[], firstName?: string): string {
  ...
  lines.push(
    ...
    /*
     * ההשתקה הייתה כפתור שנצמד לכל התראה. מרגע שהיא משפט, היא
     * חייבת להיאמר במקום אחד לפחות — יכולת שאיש אינו יודע עליה
     * שווה בדיוק כמו יכולת שאינה קיימת.
     */
    "בפגישה? כתבו לי *„שקט לשעתיים”* (או לחצי שעה, או עד מחר) ולא אפריע. *„מספיק שקט”* מחזיר אותי.",
  );
  return lines.join("\n");
}
