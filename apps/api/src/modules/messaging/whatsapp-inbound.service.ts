  ...
  async handle(payload: Record<string, unknown>, source: InboundSource = "any"): Promise<void> {
    ...
    for (const entry of parsed.data.entry) {
      for (const change of entry.changes) {
        ...
        /*
         * ‎**קו הסוכן שייך לאפליקציית הסוכן — ומכאן ומטה זה כל מה שיש.**
         *
         * הגוף חתום, אך חתום **בסוד כלשהו שהמערכת מכירה**. בלי
         * התנאי הזה סוד של אפליקציית החיבור שדלף היה מספיק כדי
         * לשלוח מטען שנושא את `phone_number_id` של קו הסוכן —
         * ומשם להריץ את הסוכן האישי בשמו של המתווך, או לרשום
         * תשובה לתזכורת סיור ולפתוח בעקבותיה התראות ומשימות.
         *
         * ‎**המיקום הוא הכל.** השומר ישב תחילה רק לפני ענף הסוכן,
         * וטיפול הלחיצות שמעליו נשאר חשוף — כלומר הגבול היה קיים
         * למחצה, וזה הסוג הגרוע (ביקורת Codex). הוא נמצא עכשיו
         * מעל **כל** טיפול שנוגע בקו הזה.
         *
         * ‏מה שמעליו — `account_update`, הדים, היסטוריה וסנכרון
         * אנשי קשר — הם אירועים של קווי המשרדים ושייכים דווקא
         * לאפליקציית החיבור. וקליטת הלידים שמתחת אינה מוגבלת
         * למקור: משרד שהזין את המספר שלו ידנית מגיע דרך אפליקציית
         * הסוכן, וסינון שם היה שובר אותו בשקט.
         */
        if (source === "connect" && incomingLine === assistantCreds?.phoneNumberId) {
          this.logger.warn(
            "מטען לקו הסוכן הגיע חתום בסוד של אפליקציית החיבור — נזרק בלי טיפול",
          );
          continue;
        }
