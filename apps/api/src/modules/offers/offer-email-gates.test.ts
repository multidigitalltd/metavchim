...
/**
 * ‎**מנת ההצעות מסודרת, ולכן „הראשון” בה יציב** (ביקורת Codex, P2).
 *
 * ‏`first` קובע את טוקן ההסרה שבמייל ואת מפתח האידמפוטנטיות.
 * ‏`retryPending` שולף בלי `orderBy`, ולכן בלי מיון „הראשון” היה
 * ‏יכול להיות שורה אחרת בכל סבב — המפתח היה משתנה, והשאלה לספק
 * ‏אחרי כישלון עמום הייתה מחפשת מחרוזת שאינה קיימת ושולחת שוב.
 */
describe("‏סדר המנה בשליחת ההצעות", () => {
  it("‏המנה ממוינת לפני שנבחר „הראשון”", () => {
    const source = body("deliver");
    expect(source).toMatch(/\.sort\(/u);
    expect(source).toContain("offerId");
  });

  /*
   * ‏ובמקום אחד. הטענה היא על **הקובץ** ולא על שלושת הקוראים בשמם:
   * ‏מיון שני כאן פירושו שני סדרים אפשריים לאותה מנה, וזה בדיוק
   * ‏מה שהמיון בא לסגור.
   */
  it("‏ואין מיון שני בקובץ", () => {
    expect(SOURCE.match(/\.sort\(/gu)).toHaveLength(1);
  });
});
...
describe("איפוס הסמן בכיבוי", () => {
  /*
   * ‎**השירות ולא הבקר.** הכתיבה עברה ל-`OfficeSettingsService`
   * ‏ביום שנוסף לה קורא שני — „תכבה הצעות אוטומטיות” מהוואטסאפ.
   * ‏שער שנשאר מכוון לקובץ שאינו מחזיק עוד את הקוד בודק מחרוזת
   * ‏שאין בה את מה שהוא מחפש, וזו הצורה המסוכנת ביותר של בדיקה
   * ‏ירוקה — ולכן הבדיקה הראשונה כאן היא שהקובץ באמת מכיל אותו.
   */
  const SETTINGS = readFileSync(
    join(import.meta.dirname, "..", "settings", "office-settings.service.ts"),
    "utf8",
  );
