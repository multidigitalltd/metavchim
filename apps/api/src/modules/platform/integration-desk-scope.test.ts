...
describe("שולחן החיבורים — גבול הגישה", () => {
  ...
  /*
   * ‎**הסריקה למעלה קוראת קבצים, ולכן אפשר לעקוף אותה בהאצלה.**
   *
   * שיטה בשולחן שקוראת ל-`this.calls.something()` אינה מכילה שום
   * ‎`tx.call`, עוברת את הרשימה הלבנה בשקט — ומרחיבה את הגישה בדיוק
   * כמו שכתיבה ישירה הייתה מרחיבה. זה אינו תרחיש תיאורטי: היכולת
   * „ייבוא הקלטות בשם משרד” נוגעת ב-`calls`, והדרך הקצרה להוסיף
   * אותה הייתה בדיוק כאן. היא הושמה בקובץ משלה
   * (`telephony/platform-recordings.controller.ts`) — ליד המנוע
   * שהיא מפעילה, ומצהירה בעצמה מה היא עושה.
   *
   * הגדר: השולחן מייבא **תשתית בלבד** — `common/`, `core/`,
   * ‎`config/` — ואף לא מודול אחד מהשכנים שלו. שם יושבים הלקוחות,
   * הלידים והשיחות.
   */
  it("אינו מאציל עבודה למודול שכן", () => {
    for (const file of files) {
      const text = readFileSync(file, "utf8");
      /*
       * מודול שכן הוא `../<name>/` — **רמה אחת** למעלה. התשתית
       * ‎(`../../common/` וחבריה) היא שתי רמות, ולכן היא אינה
       * נתפסת כאן: ה-`(?!\.\.\/)` הוא כל ההבדל.
       */
      const siblings = [...text.matchAll(/from "\.\.\/(?!\.\.\/)([^"]+)"/gu)];
      expect(
        siblings.map((hit) => hit[1]),
        `${file}: ייבוא ממודול שכן — ראו ההסבר בבדיקה`,
      ).toEqual([]);
    }
  });
