...
describe("‏מסלול העדכון מוסר ל-`fieldsToColumns` את השורה השמורה", () => {
  ...
  /*
   * ‏אישור „טאבו משותף” הוא מסלול כתיבה שלישי, והוא נשען על אותו
   * ‏תיקון: „כבה” על שורה שסוגה השמור הוא הסוג הישן חייב לפרוש גם
   * ‏את הסוג, אחרת התשובה נמחקת בקריאה הבאה והשורה חוזרת לרשימה.
   */
  it("‏ואישור „טאבו משותף” מוסר את `current`", () => {
    expect(SERVICE).toContain("fieldsToColumns({ sharedTabu }, current)");
  });
