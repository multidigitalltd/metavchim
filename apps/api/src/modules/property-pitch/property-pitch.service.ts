...
export class PropertyPitchService {
  ...
  private async sendOne(input: {
    ...
    /*
     * ‎**וגם על ציר הזמן של הקונה — בטרנזקציה נפרדת** (ביקורת Codex, P1).
     *
     * ‏השורה בתיבה נושאת `cardKind: "buyer"`, אבל ציר הזמן בכרטיס
     * ‏קורא `interaction` לפי `buyerId` — כלומר ההצעה יצאה, נשמרה,
     * ‏ולא הופיעה בשום מקום שהסוכן מסתכל בו כשהוא פותח את הקונה
     * ‏(בקשת המשתמש).
     *
     * ‎**ולא באותה טרנזקציה עם `sent`.** כשל בכתיבה הכבדה מהשתיים
     * ‏היה מגלגל אחורה גם את הסימון, ההודעה הייתה נשארת `pending`
     * ‏אף שיצאה — והמסך שאומר „בשליחה…” הוא בדיוק מה שגורם לסוכן
     * ‏לשלוח שוב, כלומר עותק שני אצל הלקוח. `sendState` הוא העובדה
     * ‏שקובעת אם מותר לשלוח שוב; הציר הוא נוחות, ואטומיות ביניהם
     * ‏אינה שווה את המחיר. `email-inbox` כבר למד את זה בדיוק כאן,
     * ‏ומפריד בין השתיים מאותה סיבה מילה במילה.
     *
     * ‏נכתב **אחרי השליחה ולא לפניה**: ציר הזמן הוא מה שהסוכן קורא
     * ‏כדי לדעת מה נאמר ללקוח, ושורה שאומרת „נשלחה הצעה” על מייל
     * ‏שנדחה היא בדיוק התיעוד הכוזב שהמצב `failed` קיים כדי למנוע.
     */
    await this.prisma
      .withTenant((tx) =>
      ...
      .catch(() => this.logger.error(`רישום ההצעה בציר הזמן נכשל: ${messageId}`));
    return "sent";
  }
}
