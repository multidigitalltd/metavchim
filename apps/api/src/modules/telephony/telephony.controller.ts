 ...
 */
/*
 * ‎`softphone` — האם הסופטפון של הלקוח **רשום ברגע זה**.
 *
 * השרת אינו יכול לדעת זאת: הרישום הוא בין הדפדפן למרכזייה. הדגל
 * בוחר בין שני קווים של אותו משתמש — השלוחה שלו או הנייד שלו —
 * ולכן אין בו שטח תקיפה: מי שישקר יגרום לשיחה לצלצל אצלו במקום
 * הלא נכון, וזה הכול.
 */
const DialSchema = z
  .object({
