...
export class VirtualNumbersController {
  ...
  /**
   * מחיקה מותרת, אבל **הכיבוי הוא ברירת המחדל במסך**.
   *
   * מחיקה מוציאה את ההגדרה בלבד; השיחות שכבר נרשמו שומרות את
   * `dialedNumber` ולכן המדידה ההיסטורית שורדת. מה שאובד הוא השם —
   * ולכן המסך מציע לכבות, וזה מה שרוב המשרדים באמת רוצים.
   */
  @Delete(":id")
  @RequireCapability("settings.manage")
  @HttpCode(200)
