...
export type WebhookHitOutcome =
  ...
  | "no_feature"
  /*
   * ‎**נדחתה על תקרת הקצב — והיא לא הגיעה לשירות בכלל.**
   *
   * ‏השער רץ לפני הבקר, ולכן עד עכשיו הדחייה הזו לא הותירה שום
   * ‏עקבה: מי שבירר „למה השיחה לא נקלטה” ראה יומן ריק. במרכזייה
   * ‏זה סופי — רובן אינן מנסות שוב.
   *
   * ‏שורה אחת לכל מפתח בכל חלון, ולא אחת לכל בקשה: הצפה היא
   * ‏בדיוק המצב שבו זה קורה, ורישום כל דחייה היה מגביר אותה.
   */
  | "rate_limited";
