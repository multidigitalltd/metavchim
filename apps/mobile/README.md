...
| האפליקציה | מי מחובר, הפרופיל (במערכת), התראות הפוש, התנתקות, גרסה |
| **כל שאר המערכת** | דשבורד, שיחות, התאמות, הצעות, תיבת מייל, יומן, משימות, שת"פים, דוחות, פורום, המנטור, ניהול משרד, הקמה, פלטפורמה — **ה-web עצמו, מוטמע** באפליקציה (ראו „איך זה מתחבר”) |
...
1. **Releases ⟵ `mobile-latest`** — ה-APK העדכני, תמיד באותה כתובת:
   ...
   מותקן — במסך „עוד”, בשורת הגרסה. אפשר גם להריץ ידנית („Run workflow”) עם כתובת
...
app/                 ניתוב לפי קבצים (expo-router)
  ...
  today.tsx  voice.tsx  notifications.tsx  app-settings.tsx
  leads/  properties/  buyers/   index (רשימה) · [id] · new / edit
  tasks/new.tsx
  web/[...path].tsx  כל מסך של המערכת — ה-web מוטמע
src/lib/             api, auth, google-login, session-store, push, cache, use-query, nav, shell, fonts, recorder, agent, format, labels, dtos
src/components/      Text, Screen (TopBar), Drawer, AuthShell, Logo, Card, Pill, Button, Field, Row, Chips, States, ProposalCard, CacheNotice
src/theme.ts         טוקני העיצוב — עותק של globals.css (נאכף ב-verify:theme), המעטפת הכהה, סולם הטיפוגרפיה
...
- **מצב כהה** — ה-web תומך; באפליקציה הטוקנים הם של הערכה הבהירה בלבד (מסכי ה-web
  המוטמעים כן מכבדים את ערכת המכשיר).
- **מסכים נייטיביים נוספים** — כל מה שאינו נייטיבי מוצג מה-web ועובד; מה שהשטח מבקש
  הכי הרבה (יומן, משימות, התאמות) הוא המועמד הבא לגרסה נייטיבית.
