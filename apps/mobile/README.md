...
| לידים | סינון במסד, חיפוש. **ליד חדש וכרטיס הליד** — ה-web המוטמע: כל הפונקציות של המערכת (שיחות, מייל, סטטוס, ציר זמן, הערות, משימה, פגישה) |
| קול | הסוכן האישי: הקלטה → תמלול → הצעה לאישור → ביצוע; או הקלדה |
| נכסים | רשימה עם סינון וחיפוש בשרת. **נכס חדש וכרטיס הנכס** — ה-web המוטמע: אותו טופס קליטה מלא כמו במערכת, עריכה, תמונות, בדיקות, הצעות מחיר, בית פתוח, תמחור, ציר זמן, שותפים |
| לקוחות | רשימה עם סינון וחיפוש בשרת (גם בשם). **לקוח חדש וכרטיס הלקוח** — ה-web המוטמע: אותו טופס קליטה מלא כמו במערכת, עריכה, השוואה, ציר זמן, התאמות |
...
| התראות · Push | רשימה, סימון כנקרא; התראות פוש למכשיר (Expo) עם ניווט לישות; **מונה על אייקון האפליקציה** = הלא-נקראות (מהשרת בכל פוש, ומיושר בכל פתיחה וחזרה לחזית). **בלי פוש אמיתי** (עוד אין Firebase / מזהה פרויקט, או שהמשתמש לא אישר) — סריקת רקע כל ~15 דקות (`expo-background-task`, `src/lib/background-notify.ts`) מציגה התראה מקומית לכל התראה חדשה ומיישרת את המונה; מוסרת אוטומטית כשהפוש נרשם |
...
app/                 ניתוב לפי קבצים (expo-router)
  ...
  leads/  properties/  buyers/   index (רשימה)   (יצירה והכרטיס עצמו — web/[...path])
...
src/components/      Text, Screen (TopBar), Drawer, AuthShell, Logo, Card, Pill, Button, Field, Row, Chips, WhenPicker, LinkPicker, LockScreen, States, ProposalCard, CacheNotice
