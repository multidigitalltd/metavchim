...
export default function WebScreen() {
  ...
  const params = useLocalSearchParams<
    { path?: string | string[] } & Record<string, string | string[]>
  >();
  ...
  const query = Object.entries(params)
    .filter(
      ([key, value]) =>
        key !== "path" && key !== "#" && typeof value === "string",
    )
    .map(
      ([key, value]) =>
  ...
  useEffect(() => {
    ...
    apiPost<{ code: string; webOrigin?: string }>("/auth/web-session", {})
      .then(({ code, webOrigin: served }) => {
        ...
        setInitialUrl(
          `${origin}/api/v1/auth/web-session/${code}?next=${encodeURIComponent(target)}`,
        );
      })
      .catch((err: unknown) => {
        if (!cancelled)
          setError(
            err instanceof Error ? err.message : "לא הצלחנו לפתוח את המסך",
          );
      });
    return () => {
      cancelled = true;
  ...
  /*
   * ‏ערכת הנושא של האפליקציה חלה גם על העמוד המוטמע: בטעינה (הסקריפט
   * ‏שרץ לפני התוכן) ובכל שינוי בזמן שהעמוד פתוח (החלפה בהגדרות,
   * ‏או מצב לילה של המכשיר שהתחלף).
   */
  const bridge = themeBridgeScript(choice, scheme);
  useEffect(() => {
    webview.current?.injectJavaScript(bridge);
  }, [bridge]);
    ...
    <Screen title={title} scroll={false} flush root={pathname === "/"}>
      {error ? (
        ...
        <WebView
          ...
          onMessage={(event) => {
            // ‏המשתמש בחר ערכה מתוך עמוד הפרופיל — האפליקציה מיישרת קו
            const next = parseThemeMessage(event.nativeEvent.data);
            if (next !== null && next !== choice) setChoice(next);
          }}
          ...
          onError={(event) =>
            setError(
              event.nativeEvent.description || "המסך לא נטען — בדקו את הרשת",
            )
          }
          onHttpError={(event) => {
            if (event.nativeEvent.statusCode >= 500)
              setError("השרת החזיר שגיאה — נסו שוב");
          }}
          onOpenWindow={(event) => {
