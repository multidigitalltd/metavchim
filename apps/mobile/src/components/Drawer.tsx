...
import { chrome, radius, space, type } from "@/theme";
...
const TAG_TEXT: Record<NavTag, string> = {
  soon: "בקרוב",
  beta: "בטא",
  ai: "AI",
};
          ...
          <SafeAreaView style={styles.fill} edges={["top", "bottom", "right"]}>
            ...
            >
              {NAV_ITEMS.filter((item) =>
                navVisible(item, user ?? null, summary),
              ).map((item) => {
...
const useStyles = makeStyles((t) => {
  ...
  return {
    ...
    panel: { width: WIDTH, backgroundColor: ch.sidebarBg, paddingBottom: 14 },
    head: {
      padding: 22,
      paddingBottom: 18,
      borderBottomWidth: 1,
      borderBottomColor: ch.sidebarLine,
      gap: 5,
    },
    tenant: { color: ch.sidebarMuted, fontSize: type.caption, minHeight: 18 },
    ...
    linkPressed: { backgroundColor: ch.sidebarActiveBg },
    label: { flex: 1, color: ch.sidebarFg, fontSize: type.body },
    count: {
      color: ch.sidebarMuted,
    ...
    badgeText: { color: ch.badgeFg, fontSize: type.caption, lineHeight: 18 },
    tag: {
      borderWidth: 1,
    ...
    tagText: { color: c.action, fontSize: 12, lineHeight: 16 },
    user: {
      marginHorizontal: 12,
      padding: 12,
      backgroundColor: ch.sidebarUserBg,
      borderRadius: 11,
      flexDirection: "row",
      alignItems: "center",
      gap: 10,
    ...
    userName: { color: ch.sidebarFg, fontSize: type.bodySm },
    userRole: { color: ch.sidebarMuted, fontSize: 12.5, lineHeight: 16 },
  };
});
