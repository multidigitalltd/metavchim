...
import { colors, radius, space, TOPBAR_H, type } from "@/theme";
...
export function Screen({
  ...
  const sub = trailing ? <View style={styles.trailingRow}>{trailing}</View> : null;
  if (!scroll) {
    return (
      <SafeAreaView style={styles.safe} edges={["top", "left", "right"]}>
        {topbar}
        {sub}
        <View style={[styles.fill, flush ? null : styles.fillPadded]}>{children}</View>
      </SafeAreaView>
    );
  }
  return (
...
export function TopBar({ title, root }: { title: string; root: boolean }) {
  ...
  return (
    <View style={styles.topbar}>
      ...
      <Text style={styles.title} weight={900} numberOfLines={1} accessibilityRole="header">
        {title}
      </Text>
      <View style={styles.end}>
        ...
        {hasFeature("voice_intake") ? (
          ...
          >
            <Ionicons name="mic-outline" size={18} color={colors.onAction} />
          </Pressable>
        ) : null}
      </View>
    </View>
...
const styles = StyleSheet.create({
  ...
  content: { paddingHorizontal: 16, paddingTop: 24, paddingBottom: 44, gap: space.md },
  flush: { flexGrow: 1 },
  trailingRow: { flexDirection: "row", justifyContent: "flex-end", paddingHorizontal: 16, paddingTop: 12 },
  topbar: {
    height: TOPBAR_H,
    flexDirection: "row",
    alignItems: "center",
  ...
  title: { flex: 1, fontSize: type.screenTitle, letterSpacing: -0.4, color: colors.text },
