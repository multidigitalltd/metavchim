...
for (const theme of Object.keys(THEME_SELECTORS)) {
  ...
  /*
   * ‎**כל דומיין נמדד מול הרקע של עצמו.**
   *
   * חמשת הדומיינים הם הזוגות שהכי קל לשבור בלי לשים לב: הם נבחרים
   * לפי משמעות („סגול = התאמות”), והשינוי בהם נעשה בדרך כלל
   * מטעמי מראה — „הכתום חזק מדי”, „הסגול לא מספיק עמוק”. כיוונון
   * כזה מזיז `fg` ו-`bg` בנפרד, ומי שמכוונן רואה ריבוע צבע ולא
   * יחס. בלי שער, אריח KPI שלם יכול לרדת מתחת לסף בלי סימן.
   *
   * הצירוף נמדד בשלוש הערכות: הערכים הכהים נגזרו כאן ולא הגיעו
   * מהחבילה, ולכן דווקא הם צריכים מדידה ולא הבטחה.
   *
   * ‎`tile` ו-`line` אינם כאן — הם אריח ומסגרת, כלומר אלמנטים
   * גרפיים שסִפָּם 3:1 ואינם נושאים טקסט. `line` נבדק ממילא כגבול
   * במקום שבו הוא מקיף פקד.
   */
  for (const domain of ["green", "peach", "violet", "blue", "neutral"]) {
    REQUIRED.push([
      theme,
      `domain-${domain}-fg`,
      `domain-${domain}-bg`,
      4.5,
      `דומיין ${domain} (${suffix})`,
    ]);
  }
  /* מספרי נתונים — הרצפה שהחבילה נוקבת בה במפורש */
  REQUIRED.push([theme, "color-data-muted", "color-surface", 4.5, `מספרי נתונים (${suffix})`]);
}
