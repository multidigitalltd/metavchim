...
if (bulkBar !== null && listBody !== null && listCard !== null) {
  ...
  /*
   * ‎**הפעולות מעל הסינונים** (בקשת בעל המוצר).
   *
   * ‏מי שסימן נכסים מחפש את הפעולה, לא את המסננים — ולכן הסינונים
   * ‏ירדו מתחת לסרגל. ההכרעה היא על **הסדר**, ולכן היא נמדדת בין
   * ‏שניהם.
   */
  const filterRow = onlyIn(
    PROPERTIES,
    'label="סינון לפי סטטוס"',
    "שורת הסינונים",
    "עמוד הנכסים",
  );
  if (filterRow !== null && bulkBar > filterRow) {
    problems.push("שורת הסינונים חזרה מעל סרגל הפעולות — מי שסימן מחפש את הפעולה, לא את המסננים");
  }

  /*
   * ‎**הקו מפריד את פס הפקדים מהטבלה שמתחתיו.** ‎`border-t` היה
   * מפריד אותו מהכותרת שמעליו ומדביק אותו לשורה הראשונה — כלומר
   * פס שנראה כמו השורה הראשונה של הטבלה.
   *
   * ‎**והוא נמדד על השורה האחרונה שלפני הטבלה, לא על הסרגל.**
   * ‏קודם היה זה הסרגל; אחרי שהסינונים ירדו מתחתיו, הצמדת הבדיקה
   * ‏אליו הייתה דורשת קו באמצע פס הפקדים ומשאירה את הטבלה דבוקה
   * ‏לשורה שמעליה — כלומר שער שאוכף את ההפך מכוונתו.
   */
  const lastControls = filterRow ?? bulkBar;
  /*
   * ‎**בלי ההערות.** ההערה שמסבירה למה הקו עבר לכאן מכילה את
   * ‏המחרוזת „border-b” בעצמה, ולכן מחיקת המחלקה עברה את השער
   * ‏בשקט: מדדתי טקסט ולא מחלקה.
   */
  const strip = PROPERTIES.slice(Math.max(0, lastControls - 700), lastControls)
    .replace(/\{\/\*[\s\S]*?\*\/\}/gu, "")
    .replace(/\/\*[\s\S]*?\*\//gu, "");
  if (!/className="[^"]*border-b/u.test(strip)) {
    problems.push("פס הפקדים אינו נפרד בקו מהטבלה שמתחתיו (`border-b`)");
  }
}
