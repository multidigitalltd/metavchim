...
const RULES = [
  ...
  },
  {
    ...
    tagScan: true,
    /*
     * ‎**למעט תיבה בגודל קבוע.**
     *
     * הכלל הזה אומר „גודל קבוע מכבה את הסקלה”, והכלל שמתחתיו אומר
     * „גודל שגדל בתוך תיבה קבועה גולש”. אלמנט שהוא גם `mv-` וגם
     * תיבה קבועה נפל בין שניהם: **לא הייתה לו אף אפשרות חוקית.**
     * השער שלי עצמו גילה את הסתירה כשתיקנתי את המונוגרם בהגדרות.
     *
     * ההכרעה היא לפי מה שמקיף את הטקסט: כשהתיבה קבועה, גודל קבוע
     * הוא הנכון — וזה בדיוק מה ש-`.mv-avatar-dot` עושה מאז שהתקלה
     * הזו תוקנה בפעם הראשונה.
     */
    tagWhen: (tag) => tag.includes("mv-") && !/\b(?:width|height): \d/u.test(tag),
  ...
  {
    /*
     * ‎**טקסט שגדל בתוך תיבה שאינה גדלה.**
     *
     * ההמרה של הגדלים המוטבעים הייתה נכונה בערך ושגויה בהקשר בשישה
     * מקומות: מונוגרם בעיגול של 30 או 48 פיקסלים קבועים, שבסקלה
     * 200% גולש מהעיגול (ביקורת Codex). זו אותה תקלה שכבר תוקנה
     * פעם אחת ב-`globals.css`, וחזרה בשכבת ה-TSX כי ההמרה שאלה „מה
     * הגודל” ולא „מה מקיף אותו”.
     *
     * ‎**המרפא אינו אחיד**: תיבה שהיא סימן דקורטיבי נשארת קבועה על
     * שני חלקיה (כמו `.mv-avatar-dot`), ותיבה שנושאת ערך נתונים
     * גדלה יחד עם הערך. הכלל אינו מכריע ביניהם — הוא רק מסרב
     * לצירוף שבו אחד גדל והשני לא.
     */
    tagScan: true,
    tagWhen: (tag) => /\b(?:width|height): \d/u.test(tag),
    pattern: /fontSize: "(?:var\(--type-[a-z0-9-]+\)|calc\([^"]*rem[^"]*\))"/gu,
    fails: () => true,
    describe: (m) =>
      `${m[0]} בתוך תיבה ברוחב/גובה קבוע — הטקסט גדל והתיבה לא`,
  },
  {
    /*
     * ‎**טקסט ב-SVG נמדד ביחידות ה-viewBox, לא בפיקסלים של המסך.**
     *
     * ‎`DonutChart` מצייר ב-`viewBox` של 140×140 ושתי שורות המרכז
     * שלו מרוחקות 20 יחידות זו מזו. גודל שגדל עם השורש הופך אותן
     * ל-52 ול-28 יחידות — הן נדרסות זו על זו ומכסות את הטבעת
     * (ביקורת Codex). הגיאומטריה קבועה, ולכן גם הכיתוב שבתוכה.
     *
     * הגדלת התרשים כולו היא שינוי תכן ולא תיקון גודל, והיא אינה
     * נדרשת כאן: הערכים מופיעים גם בשורות המדד שלצד התרשים.
     */
    tagScan: true,
    tagWhen: (tag) => /^<(?:text|tspan)\b/u.test(tag),
    pattern: /fontSize: "(?:var\(--type-[a-z0-9-]+\)|calc\([^"]*rem[^"]*\))"/gu,
    fails: () => true,
    describe: (m) => `${m[0]} בתוך SVG — יחידות ה-viewBox קבועות`,
  },
  {
...
for (const scope of SCOPED) {
  ...
  for (const file of found) {
    ...
    for (const rule of RULES) {
      ...
      if (rule.tagScan) {
        /*
         * ‎`tagWhen` — **איזה תג מעניין את הכלל הזה.**
         *
         * התנאי היה קבוע בלולאה: „רק תגים שיש בהם `mv-`”. זה נכון
         * לכלל שנכתב עליו, אבל כשהוסף כלל שני התנאי הזה חל עליו גם
         * כן, ובשקט — וכך תג שמחלקתו מגיעה במשתנה
         * ‎(`className={className}`) לא נבדק כלל. תנאי לכל כלל הוא מה
         * שמונע מהאיחוד הזה לחזור.
         */
        const wants = rule.tagWhen ?? ((tagText) => tagText.includes("mv-"));
        for (const tag of jsxTags(text)) {
          if (!wants(tag.text)) continue;
