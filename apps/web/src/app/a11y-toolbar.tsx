...
/**
 * ‎**כפתור הנגישות — בצד של כל מסך, כולל המסכים הציבוריים.**
 *
 * ## למה הוא חזר
 *
 * הגרסה הקודמת הסירה את הכפתור הצף והעבירה את ההתאמות לעמוד
 * הפרופיל, מתוך נימוק נכון לחצי מהמשתמשים: מתווך שעובד במערכת
 * כל היום מגדיר פעם אחת ולא חוזר. אבל שלושה קהלים נשארו בלי דרך
 * להגיע להתאמות בכלל:
 *
 * - ‎**מי שאינו מחובר.** דף ההצעה, דף החתימה, דף הנחיתה של הנכס
 *   ומסך ההתחברות פונים לציבור הרחב — ולאף אחד מהם אין פרופיל.
 * - ‎**מי שלא יודע שיש פרופיל.** לקוח קצה או סוכן ביום הראשון
 *   מחפש את הסמל המוכר בצד המסך, לא תפריט משתמש.
 * - ‎**מי שהתקן מדבר עליו.** ת"י 5568 (על בסיס WCAG 2.2) מצפה
 *   שההתאמות יהיו זמינות מכל עמוד, וההצהרה המשפטית של המערכת
 *   הבטיחה בדיוק את זה.
 *
 * הפרופיל נשאר — מי שמעדיף מסך מלא ומסודר מוצא שם את אותם מתגים.
 * שני המקומות כותבים דרך `commitA11y` ומאזינים לאותו אירוע, ולכן
 * לעולם אינם מציגים שני מצבים שונים.
 *
 * ## איפה הוא יושב
 *
 * בקצה השמאלי, מתחת ללשונית התמיכה — ולא בפינה התחתונה, שבה יושב
 * פס הסופטפון. במסך צר שתי הלשוניות יורדות לפינה, אחת מעל השנייה.
 *
      ...
      {open ? (
        ...
        >
          ...
          <p className="m-0 mb-4 text-[length:var(--type-caption)]" style={{ color: "var(--color-text-muted)" }}>
            כל התאמה חלה מיד. למשתמש מחובר היא נשמרת בחשבון ומלווה אותו בכל מכשיר.
          </p>
