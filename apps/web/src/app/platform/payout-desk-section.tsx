  ...
  return (
    <section aria-labelledby="payout-desk-heading" className="mb-8">
      <h2 id="payout-desk-heading" className="mb-1 text-lg font-semibold">
        ...
        {pending.length > 0 ? (
          <span className="mv-chip ms-2" style={{ fontSize: 12 }}>
          ...
          {rows.map((row) => (
            ...
            >
              <div className="flex flex-wrap items-baseline gap-2">
                <b className="text-[15px]">{shekels(row.amountAgorot)} ₪</b>
                <span>{row.tenantName}</span>
                <span style={{ color: STATUS_COLOR[row.status], fontWeight: 700, fontSize: 12.5 }}>
              ...
              {row.status === "pending" || row.status === "approved" ? (
                <>
                  <button
                    type="button"
                    className="mv-btn-plain mt-1 text-[12.5px]"
                  ...
                  {openId === row.id ? (
                    <dl
                      className="mt-1 grid gap-x-3 text-[12.5px]"
