          ...
          */}
          ...
          <div className="mb-3">
            ...
            <p className="mb-2 text-sm" style={{ color: "var(--color-text-muted)" }}>
              ...
              מותר. נדרשת תבנית מסוג Utility עם <b>חמישה משתנים</b> בגוף —
              ‎{"{{customer_name}}"}‎, ‎{"{{visit_date}}"}‎,
              ‎{"{{visit_time}}"}‎, ‎{"{{visit_address}}"}‎
              ו-‎{"{{office_name}}"}‎. שדות ולא נוסח אחד: תבנית שגופה משתנה
              יחיד אינה קריאה ל-Meta, והיא מסווגת כ-Marketing מה שאינה
              מבינה. <b>הנוסח שניסחתם באוטומציות ממשיך לצאת במייל</b>;
              בוואטסאפ יוצאים השדות. בלי תבנית מוגדרת מי שאין לו מייל מגיע
              כמשימה לסוכן. <b>בלי כפתור</b>: הנמען הוא לקוח, ואין לו מה
              לפתוח במערכת.
            </p>
            <div className="flex flex-wrap gap-2">
              <input
