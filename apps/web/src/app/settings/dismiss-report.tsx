    ...
    <section aria-labelledby="dismiss-report" className="mb-8">
      ...
      {report.total === 0 ? (
        ...
        >
          ...
          <ul className="m-0 list-none p-0">
            {report.tallies.map((tally) => (
              <li key={tally.reason} className="mb-2">
                <div className="mb-0.5 flex items-baseline justify-between gap-2 text-[length:var(--type-body-sm)]">
                  <span>
                    {tally.label}
                    {tally.criterion !== null ? (
                      <span className="ms-1.5 text-[length:var(--type-caption)]" style={{ color: "var(--color-text-muted)" }}>
          ...
          {report.insight !== null ? (
            <p
              className="m-0 mt-3 rounded-lg border p-3 text-[length:var(--type-caption-lg)]"
