...
export function TelephonySection() {
  ...
  return (
    <section className="mv-list-card px-5 py-[17px]" aria-labelledby="telephony-heading">
      ...
      {status.connected && provider?.clickToDial ? <MyLine /> : null}
    </section>
  );
}

/**
 * קו ה-SIP האישי — לסופטפון.
 *
 * הוא כאן ולא בפרופיל כי הוא חסר משמעות בלי המרכזייה שמעליו, והמסך
 * הזה הוא היחיד שמסביר מה הוא. הוא מוצג לכל סוכן ולא רק למנהל:
 * לדבר עם לקוח זו העבודה, ולא הגדרה של מנהל.
 */
function MyLine(): React.JSX.Element {
  const [line, setLine] = useState<{ username: string; hasPassword: boolean } | null>(null);
  ...
  function load(): void {
    apiGet<{ username: string; hasPassword: boolean }>("/settings/telephony/my-line")
      .then((res) => {
        setLine(res);
        setUsername(res.username);
      })
      .catch(() => setLine({ username: "", hasPassword: false }));
  }
  ...
  return (
    <form onSubmit={(e) => void save(e)} className="mt-4 max-w-md border-t pt-4" style={{ borderColor: "var(--color-border)" }}>
      <p className="m-0 mb-1 text-[13.5px] font-bold">קו ה-SIP שלי (לדיבור מהדפדפן)</p>
      <p className="m-0 mb-3 text-[12.5px]" style={{ color: "var(--color-text-muted)" }}>
        הקו הזה הוא <b>שלכם ולא של המשרד</b> — שיחה נכנסת מצלצלת אצל מי שהיא מיועדת לו.
        קבלו אותו ממנהל המרכזייה, והוא מה שמאפשר לדבר עם אוזניות בלי להרים טלפון.
      </p>
