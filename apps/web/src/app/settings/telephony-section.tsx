...
interface ImportResult {
  found: number;
  linked: number;
  /** שיחות שלא נענו — הנתיב נשמר, אך אין מה לתמלל בהן. */
  skipped: number;
  alreadyHad: number;
  withoutCall: number;
    ...
    <div className="mt-4 rounded-xl border p-3" style={{ borderColor: "var(--color-border)" }}>
      ...
      {result ? (
        <Notice tone="success">
          ...
          {/*
            ‎**נספרות בנפרד, ולא בתוך „סומנו למשיכה”.** ההבטחה
            „ייכנסו לכרטיסים תוך כמה דקות” אינה חלה עליהן: הסבב
            מדלג על שיחה שלא נענתה בכוונה. ספירה משותפת הייתה
            הופכת את המשפט שמעל לשקר.
          */}
          {result.skipped > 0
            ? ` ${result.skipped} שייכות לשיחות שלא נענו — אלה לא יימשכו, אין בהן מה לתמלל.`
            : ""}
          {/*
