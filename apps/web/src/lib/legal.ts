...
/**
 * הכתובת הציבורית של המערכת.
 *
 * **מערכת ענן, התקנה אחת.** אין התקנות פרטיות בדומיין של הלקוח,
 * ולכן כל מקום בתיעוד שאומר „הדומיין שלכם” הוא לא גמישות אלא
 * חוסר — הקורא מקבל תבנית במקום כתובת שאפשר להעתיק, וצריך לנחש
 * מה למלא במקום שאין בו מה לבחור.
 *
 * מחוץ ל-`LEGAL` בכוונה: הפרטים שם נערכים ב-/platform ונדרסים
 * בזמן ריצה, והכתובת הזו אינה נתון שמשרד עורך אלא עובדה על
 * המוצר.
 */
export const APP_URL = "https://app.metavchim.co.il";
