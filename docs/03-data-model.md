...
- **email_reply_tokens** — הטוקן שבכתובת ה-Reply-To, שממנו נגזרים הדייר, הלקוח **והסוכן ששלח** את ההודעה שעליה עונים. **מחוץ ל-RLS במודע** (דפוס `lead_webhooks`): הוא מה שמזהה את הדייר, לפני שיש הקשר דייר. אין בו PII, ומותרים כמה טוקנים לאותו לקוח כדי שכתובות ישנות ימשיכו לעבוד אחרי מיזוג כרטיסים.
