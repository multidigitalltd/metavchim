...
- **tenants** — סוכנות: **`customer_no`** (מספר הלקוח), שם, מסלול, סטטוס, הגדרות (JSONB: מיתוג, שעות פעילות, שפה), `kanko_enabled`, הגדרות שיתוף.
  - ‎**`customer_no` — מה שאפשר להקריא בטלפון.** ה-ULID מזהה שורה; הוא אינו נאמר, אינו מוקלד ואינו נזכר. המספר מחולק מרצף במסד (`tenant_customer_no_seq`, אותו דפוס של `support_reference_seq`) — הרצף הוא מה שמבטיח שכל מספר מחולק פעם אחת ואינו חוזר. עמודה ולא נגזרת מסדר כלשהו: מספר לקוח שמשתנה כשמשרד נמחק גרוע ממספר שאינו קיים. מוצג ברשימת המשרדים ב-`/platform` ובהגדרות המשרד עצמו, כדי שגם מי שנשאל „מה מספר הלקוח שלכם” יוכל לקרוא אותו.
  - ‎**ומעל הרצף — תמורה, כדי שהמספר לא יסגיר כמה משרדים יש.** הרצף עצמו חילק 100000, 100001, 100002: משרד שנרשם וראה „100004” ידע מיד שהוא הרביעי, ושתי ספרות מתוך שש הספיקו לספר את זה — וגם שני משרדים שמשווים מספרים ידעו כמה הצטרפו ביניהם. `tenant_customer_no()` היא רשת Feistel בת ארבעה סבבים עם הליכה מחזורית, שמפזרת את הרצף על **900,000 המספרים שבין 100000 ל-999999**: שש ספרות תמיד, וכולן פעילות. היא **חד-חד-ערכית על כל התחום** — 900,000 קלטים ⇐ 900,000 פלטים שונים — ולכן הייחודיות נשארת תכונה של המנגנון ולא של מזל, בניגוד ל-`random()` עם ניסיון חוזר. משרד 900,001 יתנגש ויֵיפול על האינדקס הייחודי בקול. המשרדים שכבר קיבלו מספר מוספרו מחדש דרך אותה פונקציה, כדי שלא יהיה „כלל להיסטוריה” לצד „כלל לעתיד”.
  - ‎**`settings.boardVisibleToAgents` — מי רואה את „המשרד שלנו”.** המסך היה פתוח ל-`users.manage` בלבד, מפני שהוא מדרג את **כל הסוכנים בשמם**. אבל תכליתו הפוכה — להיתלות על הקיר ולהישלח לצוות — ובמשרד שרוצה תחרות גלויה הסגירה מרוקנת אותו מתוכן. לכן זו **הגדרה של משרד ולא יכולת של תפקיד**: בעל הסוכנות מסמן תיבה בראש המסך עצמו (התיבה מוצגת למחזיקי `settings.manage` בלבד, ונשמרת דרך `PATCH /settings/tenant` עם שאר הדגלים — מתחת לאותה נעילת שורה שמגנה על מסמך ה-JSON היחיד).
    - ‎**חסר = סגור**, כלומר בדיוק ההתנהגות שהייתה עד היום: אף משרד קיים אינו מגלה בוקר אחד שכל סוכניו רואים את הדירוג של כולם. חשיפה של נתונים על אנשים דורשת מעשה מפורש, גם בתוך אותו משרד.
    - ‎**התנאי הוא `canSeeOfficeBoard` בחבילה המשותפת**, ושם המפתח הוא `BOARD_VISIBILITY_KEY` — ארבעה קוראים (השער בנתיב, הקריאה בהגדרות, `/auth/me` והכתיבה) ומקור אחד.
    - ‎**האכיפה בשרת ולא בדקורטור.** רשימת יכולות סטטית אינה יכולה לבטא דגל ששמור בשורת המשרד, ולכן `GET /analytics/board` הוא `@AnyAuthenticated` ששורתו הראשונה היא `assertBoardVisible()`. שער בבדיקות (`office-board-scope.test.ts`) מוודא שהשורה לא תיעלם — מחיקתה הייתה משאירה את הטבלה פתוחה לכל מי שמחובר למשרד. הדגל נקרא מהמסד בכל בקשה ולא מה-Session הממוטמן, כדי שכיבוי ייכנס לתוקף מיד.
...
- **users.office_digest_opted_out_at** — ‏ביטול הצטרפות אישי לסיכום החודשי בוואטסאפ. ‏בתחילת כל חודש נשלחת לכל סוכן סקירה של החודש שנגמר: הלידים, הנכסים, הפגישות והעסקאות **שלו**, ‏והמקום שלו במשרד („3 מתוך 7”) — **בלי המספרים של אף אחד אחר** (הכרעת בעל המוצר). עמוד „המשרד שלנו” ‏מנהלי בלבד, וזו הפעם הראשונה שסוכן רואה משהו ממנו; „הדירוג המלא לכולם” היה הופך הודעה חודשית ‏להשפלה פומבית של מי שאחרון.
  - ‎**ההתראה היא גם מנעול הפעם-אחת.** אין טבלה חדשה: `notifyOnce` כותב עם מפתח ‏`office_digest:<חודש>:<סוכן>`, וה-`ON CONFLICT` הוא מה שמבטיח הודעה אחת לכל סוכן לכל חודש — גם אם ‏הסבב ירוץ עשר פעמים ביום. **נרשם קודם, נשלח אחר כך**: שליחה שקודמת לכתיבה פירושה שקריסה באמצע ‏שולחת שוב בסבב הבא.
  - ‎**ואין תנאי „היום הראשון בחודש”.** שרת שהיה למטה באותו יום היה מפספס את החודש כולו; הדדופ מטפל ‏בזה בלי לאבד דבר, והסבב הראשון שרץ אחרי תחילת החודש הוא ששולח.
  - ‎**הוויתור משתיק את הטלפון בלבד.** שתי שאלות נפרדות: `digestSkipReason` שואלת ‏האם יש מה לסכם וחוסמת הכול, ו-`digestWhatsappSkip` שואלת האם לדחוף לטלפון. ‏כשהן היו פונקציה אחת הוויתור השתיק גם את הפעמון — בניגוד למה שהמסך מבטיח ‏ולמה שכתוב כאן. **גוף ההתראה הוא הסיכום עצמו**: עמוד ההתראות מציג פרטים מ-`body` ‏בלבד, ולכן הפעמון הוא המסירה העמידה והוואטסאפ הוא הדחיפה שמעליה.
  - ‎**השליחה על קו הפלטפורמה, לא על חיבור המשרד.** העוזר האישי עונה לסוכן על קו ‏הפלטפורמה, ושם גם נוצר הקישור — שליחה על חיבור המשרד היא מספר אחר שרוב ‏הסוכנים מעולם לא כתבו אליו. ומכיוון שהסבב רץ בתחילת החודש ולא בתגובה לכלום, טקסט ‏חופשי נשלח ראשון ותבנית מאושרת (`whatsappOfficeDigestTemplate`) נופלת אל מחוץ לחלון 24 ‏השעות — אותו סדר בדיוק של התראת „לקוח ענה במייל”.
  - ‎**החודש נגזר מהמפתח עצמו** (`digestMonthAnchor`), ולא מחישוב UTC נפרד: בשעות הראשונות ‏של חודש ישראלי השניים חלוקים, והתוצאה היתה מספרים של חודש אחד תחת כותרת ‏של אחר, נעולים לצמיתות במפתח הדדופ.
  - ‎**והדיווח למנהל הוא התראה, לא שורת לוג.** מי לא קיבל בוואטסאפ ולמה — למחזיקי ‏`users.manage` בלבד (הדיווח נוקב בשמות עמיתים), עם מפתח דדופ משלו כדי שמנהל שהוא ‏גם סוכן יקבל את שתי ההודעות.
  - ‎**חודש בלבד**, ולא רבעון ושנה (הכרעת בעל המוצר): שליחה על שלושתם פירושה שלוש הודעות באותה דקה ‏בסוף דצמבר. החודש הוא גם התקופה שיעדי המנטור מוגדרים לפיה.
  - ‎**מי שלא מקבל — המנהל רואה למה.** סוכן בלי וואטסאפ מקושר וסוכן שכיבה מדווחים בשם ובסיבה; ‏שתיקה כאן היא בדיוק מה שהכלל „אין בליעת כישלון” קיים כדי למנוע. „לא הייתה פעילות” אינו מדווח — ‏זה כבר כתוב בטבלה מול הסוכן.
  - ‎**הכיבוי הוא של הסוכן ולא של המנהל** (`@AnyAuthenticated`, כותב את `TenantContext.current().userId` ‏בלבד), והוא מכבה את ההודעה לטלפון — לא את ההתראה בפעמון. חותמת ולא בוליאני: „מתי ביקש” היא שאלה ‏שנשאלת.
...
- **interactions** — ציר זמן אחוד: שיחה (עם קישור להקלטה+תמלול+סיכום), הודעת וואטסאפ, פגישה, הערה. `interaction_type`, `direction`, `payload JSONB`. `property_id` אופציונלי — פנייה על נכס מסוים (הצעה חוזרת אחרי הורדת מחיר).

### נכסים
- **properties** — עיר, שכונה, רחוב, סוג (כולל תשעה ענפים מסחריים — חנות, משרד, מחסן, תעשייה, מרתף, בניין, מרלו"ג, חניה, תחנת דלק — לצד `commercial` שהוא „מסחרי שלא נאמר איזה”; בהתאמה המטרייה מכסה כל ענף **בשני הכיוונים**, אחרת פיצול הסוגים היה פוסל בשקט כל קונה קיים שסימן „מסחרי” — ראו `logic/commercial-types.ts`), חדרים, מ"ר, קומה/מתוך, מחיר + גמישות, תאריך כניסה, בלעדיות (+תוקף), סטטוס (draft/active/on_hold/sold/rented/archived), בוליאנים נפוצים (מעלית, חניה, ממ"ד, מרפסת, מחסן), חזית/עורף, מצב הנכס, `attributes JSONB`, `readiness_score` (0–100, מחושב), `marketing_title`, `marketing_description`, `owner_contact_id`, `agent_user_id`, הערות פנימיות. `previous_price_agorot` + `price_changed_at` — המחיר הקודם ומועד השינוי; ירידה פולטת `property.price_dropped`, והכרטיס מציע לפנות שוב לקונים שביקרו ואמרו „המחיר גבוה” או שדחו בגלל המחיר (30 יום).
  ...
  - `facing` — **חזית / עורף / שניהם** (`front` · `rear` · `both`, `NULL` = לא צוין). אחת השאלות הראשונות בטלפון, ועד עכשיו לא היה לה שדה: מי ששאל רשם בהערות הפנימיות, ומשם זה לא חוזר לאף מסך ולא לשום סינון. **שלושה ערכים ולא שניים** — דירה שפונה לשני הכיוונים היא מצב נפוץ, ובבחירה של „חזית או עורף” בלבד היא נרשמת בשקר, בדיוק כמו שתאריך כניסה לבדו אילץ להמציא יום. ‎`NULL` הוא האמת על כל השורות הקיימות, ולכן אין ברירת מחדל: היא הייתה קובעת עליהן עובדה שאיש לא בדק. הריקון חזרה ל„לא צוין” נוסע ב-`clearFields` ולא כ-`null` בשדה, כמו מספר הבית — `PropertyFieldsSchema` אינו מקבל `null`, וההפרדה הזו מונעת ריקון בכל נתיב שאיש לא ביקש. אילוץ `CHECK` על שלושת הערכים: ערך שאיש אינו יודע לקרוא גרוע מחוסר.
  - `condition` — **מצב הנכס** (`new` · `renovated_full` · `renovated` · `good` · `needs_renovation`, ‏`NULL` = לא צוין). העמודה קיימת מההתחלה, ומה שלא היה קיים הוא **מקור אמת אחד לערכים שלה**: ‏הסכימה הכירה ארבעה, קטלוג הסוכן החזיק רשימה משלו, כרטיס הרשת החזיק מפת תוויות עברית, ‏ומילון הייבוא החזיק את המיפוי ההפוך — ארבעה מקומות שמי שמוסיף ערך היה צריך לזכור, ומי ‏שמשנה ניסוח היה משנה באחד מהם. עכשיו `PROPERTY_CONDITIONS` ו-`PROPERTY_CONDITION_LABELS` ‏מוצהרים פעם אחת ב-`schemas/property.ts`, והשלושה נגזרים מהם; בדיקה סורקת את המאגר ונכשלת ‏על מפת תוויות עברית שנייה לאותם ערכים.
    - ‎**„משופץ מהיסוד” הוא ערך ולא ניסוח**, וזה הערך החמישי שנוסף (בקשת בעל המוצר): שיפוץ ‏יסודי הוא הפרש של מאות אלפי שקלים מול צביעה והחלפת מטבח, וקונה ששאל „משופץ?” מתכוון לאחד ‏משניהם. הסדר ברשימה הוא מהטוב לפחות טוב, ולכן הצ׳יפים בטופס נקראים כסולם.
    - ‎**ואין `CHECK`, בשונה מ-`facing`.** `facing` נולד עם שלושת ערכיו, ו-`condition` קדם ‏לקטלוג — אילוץ היה פוסל שורות קיימות שאיש לא ראה. הסכימה היא השער בכתיבה, והמסד אוכף רק ‏`VARCHAR(20)`. מכאן גם `preserved`: ערך רפאים שהסכימה מעולם לא קיבלה ולכן שום כתיבה חדשה ‏אינה יכולה לייצר, אבל שורה ישנה שנושאת אותו מוצגת „שמור” ולא „preserved” באנגלית. הריקון ‏חזרה ל„לא צוין” נוסע ב-`clearFields`, כמו `facing` וכמו מספר הבית.
  ...
  - `partner_user_id` — **הסוכן השני על עסקה שנסגרה בשיתוף**: אחד הביא את הנכס, השני את הקונה. ‏בתוך משרד כל הסוכנים כבר רואים את כל הנכסים ואין מה „לשתף” — מה שלא נרשם בשום מקום היה **מי עוד היה שם**, ‏והמנהל ששאל „מי סגר את זה?” קיבל שם אחד (בקשת בעל המוצר).
    - ‎**עמודה ולא טבלה, ושניים ולא רשימה.** שני סוכנים הם מה שקורה בפועל; טבלת משתתפים הייתה מוסיפה ‏הצטרפות, טופס וסדר תצוגה לכל שורה כשברובן יש שם אדם אחד. `CHECK` חוסם שת״פ של סוכן עם עצמו, ואינדקס ‏חלקי מכסה רק את השורות שיש בהן שותף.
    - ‎**והניקוד בלוח אינו נוגע בזה. זו ההכרעה המרכזית** (בעל המוצר): `deals` ממשיך להיספר לפי ‏`agent_user_id` בלבד — לא נגרע מאיש ולא נכפל לאיש. מה שזה קונה הוא ש**אין מה לריב עליו**: ברגע שסימון ‏משנה דירוג, כל סימון הוא ויכוח, וסוכן שיודע שהוא מפסיד נקודות פשוט לא מסמן — כלומר השדה שנועד לתעד ‏שיתוף היה מייצר את ההפך. שדה שאינו עולה כלום מתמלא. `partner-credit-scope.test.ts` נכשל אם חלון ‏הספירה נוגע בעמודה.
    - ‎**ולכן גם שער אחר.** שינוי `agent_user_id` הוא העברת בעלות ודורש `tasks.assign`; סימון שותף הוא ‏תיעוד, ולכן מספיק `properties.edit`. מכאן `GET /properties/office-agents` — אותה שאילתה של ‏`/tasks/assignees` (‏`common/office-members.ts`, לא עותק) מאחורי יכולת אחרת. הבורר מוצג רק על נכס ‏שכבר `sold`/`rented`: על נכס פעיל אין עדיין מה לתעד.
- **property_bids** — הצעות מחיר ומו״מ על נכס: שורה לכל צעד (`side` — הצעת קונה / הצעת נגד של המוכר, `amount_agorot`, `note`), מצב שנגזר מהצעד הבא (`countered`/`superseded`) או מהכרעה (`accepted`/`rejected`/`withdrawn`); „על השולחן” = הצעד האחרון בשרשור של כל קונה. הקונה הוא קונה רשום (`buyer_id`), בלי שם חופשי. הסיכום (כמה הצעות, הגבוהה, הצעת הנגד האחרונה) נכנס לדוח למוכר.
- **open_houses** — בית פתוח על נכס: `starts_at`/`ends_at`, `slot_minutes`, `slot_capacity` (NULL = בלי הגבלה), `status` (planned/done/cancelled). המבקרים אינם טבלה: כל מבקר הוא ליד (נקלט כמו טופס דף הנחיתה) + פגישה `viewing` עם `appointments.open_house_id`, ולכן מקבל משוב סיור, מקום בדוח למוכר ותזכורת בלי קוד נוסף. ההרשמה מהדף הציבורי דרך טוקן דף הנחיתה.
- **comparisons** — דף השוואה לקונה: 2–3 נכסים זה לצד זה כתמונת מצב (`presentation`, בלי PII), קישור ציבורי לפי `public_token` עם תפוגה (60 יום), `responses` — הנכסים שהקונה סימן „מעוניין”, `open_count`/`first_opened_at`. אותם שערים כמו הצעה בודדת: הזמנה בכתב חתומה לכל נכס, ונכס שירד משיווק מוצג כ„לא זמין”.
- **property_media** — תמונות/מסמכים: S3 key, סוג, סדר, alt text. תמונה שמועלית נשמרת **אחרי שיפור** (יישור לפי EXIF, כיווץ ל-2000px, אור, חדות, WebP, בלי מטא-דאטה — כולל GPS) ובלי מקור; `enhanced_at` (NULL = הועלתה לפני הכלי, המסך מציע „לשפר”), `bytes_updated_at` (הקובץ השתנה — שיפור או טשטוש מלבנים, בלתי הפיך — ונכנס לכתובת לעקיפת המטמון). לוגו המשרד מוטבע בפינה כשההגדרה `photoLogoOverlay` דלוקה.
- **property_checks** — תיק הבדיקות לפני החתמה: שורה לכל (נכס, בדיקה) שסומנה, מהרשימה הסגורה `PROPERTY_CHECKS` (נסח, שעבודים, זכויות, תכנון, היתרים, חריגות, צווים, ארנונה, שוכר, ייפוי כוח, שטח). `status` (unchecked/ok/issue/na), הערה, מי בדק ומתי. בלי קובץ — המסמכים ב-`signed_documents`. בדיקה שטרם נעשתה הופכת למשימה עם `source_key` לפי הבדיקה.
- **recruitment_targets** — „נכסים לגיוס”: מודעה שהמשרד רודף אחריה ו**אינו מייצג** עדיין. שדות הנכס (עיר, רחוב, סוג, חדרים, מ"ר, קומה, מחיר), `source` + `source_url` (מאיפה הגיע והקישור למודעה המקורית — http/https בלבד, כי הוא מרונדר כ-`href`), `status` ממשפך הגיוס (`new`/`called`/`awaiting_reply`/`meeting_set`/`recruited`/`declined`/`lost` — ראו `logic/recruitment.ts`), שם וטלפון הבעלים כ**טקסט**, ו-`converted_property_id`. ייבוא מאקסל נכנס דרך `POST /import/recruitment` בלבד — הוא כותב לטבלה הזו ולעולם לא ל-`properties`, ולכן שורה מיובאת אינה מגיעה להתאמות, לרשת שיתופי הפעולה או להצעות עד ההמרה. טלפון בעלים או קישור שאינם תקינים יורדים מהשורה עם אזהרה במקום להפיל אותה — המודעה עצמה היא העיקר בשורת גיוס.
  ...
  - ‎**קטלוג שדות אחד למסך, לחלונית ולטופס** (`RECRUITMENT_FIELDS` ב-`logic/recruitment.ts`). לחיצה על שורה ברשימה פותחת חלונית: מה שכבר ידוע על הנכס למעלה, ומתחתיו טופס השלמה שמציג **קודם את מה שחסר** ואת מה שידוע במקטע מקופל לתיקון. החלוקה נעשית פעם אחת ב-`recruitmentFieldSplit`, וממנה ניזונים גם התצוגה וגם הטופס: שתי הכרעות נפרדות על „מה חסר” היו מציגות שדה בשני המקומות או בשום מקום. הכלל „יש ערך” מכריע שני מקרים שבדיקת אמיתות טועה בהם — **קומה 0 היא קומת קרקע** (ולא שדה ריק שיישאל שוב אחרי כל שמירה), ו-`shared_tabu = false` הוא **תשובה** ולא חוסר, בהתאם לכך שזו עמודה דו-ערכית במכוון. עמוד `‎/properties/recruitment/[id]` נשאר לקישור עמוק ולפולואפ, ומרנדר את **אותם רכיבים** — עותק שני של התצוגה כבר סטה פעם אחת, כשארבעה שדות (קומה, קומות בבניין, סוג עסקה, טאבו משותף) נוספו לטופס ולא לתצוגה.
...
- **agreements** — הזמנה בכתב והסכם בלעדיות לחתימה: הנוסח **כצילום** ברגע השליחה (`rendered_body`) ולא הפניה לתבנית, `body_hash` על מה שנחתם ו-`presented_hash` על מה שהוצג, טוקן ציבורי עם תפוגה (30 יום), ושדות החותם — שם, מספר זיהוי, תמונת החתימה, IP ודפדפן. `contact_id` nullable: הסכם **חתום** שורד מחיקת לקוח.
  - ‎`signer_template` — **הנוסח הקפוא של קישור פתוח.** קישור רגיל נוצר על לקוח ונכס ידועים, והנוסח שלו כבר מלא בשליחה; קישור פתוח (`POST /agreements/open`, ובבוט `open_signing_link`) נוצר **בלי לדעת למי הוא הולך**, ולכן שבעת השדות של `OPEN_SIGNER_PLACEHOLDERS` — שם, מספר זיהוי, כתובת, טלפון, סוג העסקה, תיאור הנכס והמחיר — נשארים בו כשורטקוד עד שהחותם ממלא אותם. ברגע החתימה אותו נוסח קפוא מורץ שוב עם מה שנמסר, ולכן ההבטחה המרכזית של המודול נשמרת: שינוי מאוחר בנוסח המשרד אינו נוגע בהסכם שכבר יצא. `NULL` = הסכם רגיל, וזה הרוב.
  - ‎**חתימה על קישור פתוח פותרת כרטיס**, ולא משאירה מסמך יתום: `findOrCreateByPhone` על הטלפון שהחותם מסר מחזירה את איש הקשר הקיים כשיש אחד — לקוח ותיק אינו מקבל כרטיס שני, וההסכם נוחת אצלו — ופותחת אחד רק כשאין. כרטיס קונה נפתח באותו תנאי, כי ההסכמים מוצגים על הכרטיס ואיש קשר בלי כרטיס הוא מסמך שנחתם ואיש במשרד אינו רואה. הבעלים הוא מי שהפיק את הקישור, אחרת הכרטיס נעלם מהסוכן שביקש אותו. הפתירה רצה תחת הקשר דייר רגיל (`withExplicitTenant`) ולא תחת פוליסת הטוקן, שהיא `SELECT` על שורה אחת — אותה הפרדה בדיוק שטופס הקליטה הציבורי עושה.
  - ‎**בלעדיות אינה נִתנת בקישור פתוח** (`agreementAllowsOpenLink`, נגזר מ-`AGREEMENT_KINDS_ON_PROPERTY`): היא נִתנת על חלקה מסוימת של המשרד, ו„בלעדיות שהלקוח מתאר בעצמו” היא מסמך על נכס שאיש לא בדק.
  - ‎`agreements_active_unique` ‏(`tenant_id, contact_id, kind, COALESCE(property_id,'')` על `pending`/`viewed`) ממשיך לחסום שני הסכמים פעילים לאותו לקוח — ואינו מפריע לקישורים פתוחים, כי `NULL` אינו שווה ל-`NULL` באינדקס ייחודי. זה גם הרצוי: שני לקוחות שנפגשו באותו יום צריכים שני קישורים.

### קונים
- **buyers** — מקושר ל-contact: ערים/שכונות מבוקשות (מערך; ולצידן `neighborhood_keys` — **עמודה נגזרת** שמחזיקה את המפתחות המקופלים של השכונות המוקלדות **ושל שמות הנעיצות על המפה**, עם אינדקס GIN חלקי: סינון הרשימה לפי שכונה אינו יכול לפתוח JSON בכל שורה, ושני המקורות שווי ערך — מי שנעץ „רמת אהרון” על המפה אמר את מה שאומר מי שהקליד אותה), תקציב מינ'-מקס', חדרים, סוגי נכס, **must_have / nice_to_have** (JSONB מובנה), `floorPreference` (קומה רצויה — **טווח או רשימה, ולעולם לא שניהם**: „משלוש ומעלה” ו„קרקע או ראשונה” הן שתי דרישות שונות בצורתן, ו-`union` מתויג מונע את השאלה „מה גובר” מלהיוולד), גמישות, מצב מימון, `maturity` (very_hot/hot/interested/not_ripe — מחושב + ניתן לדריסה ידנית), `office_status` (מזהה מתוך רשימת הסטטוסים של המשרד ב-`tenants.settings -> 'buyerStatuses'`; כל סטטוס נושא `maturity` שהוא נשען עליה, ולכן בחירה בו קובעת את שתיהן — ראו `packages/shared/logic/buyer-status.ts`), מקור, הערות AI, הערות מתווך.
...
### יומן ומשימות
- **appointments** — פגישה/סיור/שיחה: משתתפים, נכס, זמן, סטטוס, `google_event_id`, תוצאת פולו-אפ (אהב/לא מתאים/מו"מ/צריך אחר). משוב מהביקור למוכר בשלוש עמודות סגורות (`feedback_price/condition/fit` — מחיר גבוה/הוגן/נמוך, מצב טוב/דורש שיפוץ, מתאים/מיקום/גודל/תכנון); רק המספרים המסוכמים יוצאים ב„דוח פעילות” לבעל הנכס, ו-`notes` נשאר פנימי. תזכורת הסיור מסומנת ב-`reminder_sent_at` (NULL = טרם נשלחה); דחיית הפגישה מנקה אותה, כדי שהמועד החדש יקבל תזכורת משלו.
...
- **demand_follows / listing_follows** — „עקוב”, שני כיווני אותה רשת: מעקב אחרי ביקוש מתריע כשנכנס **נכס שלי** שמתאים לו, ומעקב אחרי נכס מתריע כשנכנס **קונה שלי** שמתאים לו. בלי זה מודעה שאין לה היום מה שמתאים לה היא מבוי סתום — איש אינו חוזר לגלול מודעות ישנות. המעקב הוא של הסוכן ולא של המשרד (`user_id` באילוץ הייחודי): התראה לכל הצוות על מודעה שרק אחד מהם עוסק בה היא הדרך לכבות התראות. אין מפתח זר לישות שברשת — היא תחת ה-RLS של המשרד המפרסם — והניקוי נעשה בסבב, שרואה את שני הצדדים.
...
- **contact_optout_tokens** — טוקן ההסרה מדיוור של **הכרטיס** (§30א). קיים כי מסלול ההסרה הקיים נשען על טוקן של `Offer`, ושליחה ידנית של נכס אינה יוצרת הצעה (`Offer.match_id` ייחודי — הצעה מחייבת התאמה שהמנוע קבע). **טבלה ולא עמודה על `contacts`**: עמודה הייתה מחייבת פוליסת קריאה־לפי־טוקן על הטבלה שכל ה-PII המוצפן יושב בה, וכאן אין דבר מלבד הקישור בין טוקן לכרטיס — אותו דפוס בדיוק כמו `activation_nudge_optouts`. תחת RLS עם בידוד דייר, ובנוסף פוליסת קריאה ציבורית לשורה שהטוקן שלה הוצג (`app.contact_optout_token`); ההסרה עצמה נכתבת על `contacts.opted_out_at` אחרי שהוצב הקשר הדייר מתוך אותה שורה. טוקן אחד לכרטיס ולתמיד — קישור שנשלח אתמול חייב להמשיך לעבוד — ולכן `@@unique([tenant_id, contact_id])`, והוא יורד עם מחיקת כרטיס (מוחקים גם את הקישור לאדם, לא רק את הכרטיס).
...
- **forum_threads / forum_posts** — שאלות, דיונים וטיפים בין משרדים, והתגובות עליהם. **מחוץ ל-RLS ובלי `tenant_id`**: קהילה, לא נתון של דייר. שורה אנונימית אינה נושאת מזהה מחבר כלל — רק `author_key` (HMAC) לבעלות ולכינוי „השואל/ת”; מחבר מזוהה נושא `author_user_id`/`author_tenant_id` עם `ON DELETE SET NULL`. מונים (`reply_count`, `score`), `accepted_post_id`, נעיצה/נעילה/הסתרה, אינדקס GIN לחיפוש.
- **forum_votes / forum_follows** — „מועיל” אחד לכל משתמש על כל פריט; מעקב אחרי שרשור (הערוצים ב-`users.preferences.forum`).
