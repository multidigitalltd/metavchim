...
**איך הקישור מגיע ללקוח — שני ערוצים שאינם אותו דבר.** וואטסאפ **נפתח**
בדפדפן עם הנוסח מוכן (`waUrl`), והסוכן לוחץ „שלח”; זו הדרך שעובדת תמיד, כי היא
אינה תלויה בחלון 24 השעות של Meta ולא בתבנית מאושרת. אימייל **נשלח בפועל**
מהשרת — ‏`POST /{leads|buyers}/:id/intake/send` עם `channel: "email"` — ולכן
ורק לכן הוא עובר דרך אישור נוסף במסך: פעולה שיוצאת אל לקוח ברגע הלחיצה ואי אפשר
לקחת אותה בחזרה. השליחה היא `required: true`, כלומר כישלון אצל הספק נזרק אל
הסוכן ואינו נרשם ביומן בלבד — „✓ נשלח” על מייל שלא יצא הוא בדיוק מה שגורם לא
לבדוק שוב.

**„אין מייל ללקוח” נאמר לפני הלחיצה.** `GET /{leads|buyers}/:id/intake` מחזירה
גם את הנמען (‏`recipient`), ולכן המסך יודע להשבית את האפשרות ולהסביר במקום
להציע כפתור שייכשל תמיד. השרת בודק שוב ממילא — המסך אינו שער.

**שני הערוצים שולחים את אותו קישור** (`ensure`), ולא שניים: שני טפסים פעילים
לאותו כרטיס הם השאלה „איזה מהם קובע”, ואין לה תשובה טובה.
