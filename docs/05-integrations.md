...
`clickToDial` בקטלוג הספקים מציין **קיום מימוש** ולא תמיכה של הספק.
הוא היה `true` על שלושה ספקים בלי שורת קוד שמחייגת, והמסך הבטיח
פיצ'ר שלא קורה; Zadarma ו-Voicenter מסומנים כעת `false` עד שיהיה
מימוש.
