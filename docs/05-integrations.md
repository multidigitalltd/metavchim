...
תיבה כללית = MX על תת-דומיין ייעודי (למשל `inbound.<domain>`) שמצביע ל-
`inbound.postmarkapp.com`, ואותו תת-דומיין מוגדר ב-**Inbound Domain Forwarding**
של השרת. תת-דומיין ולא הדומיין הראשי: MX על הראשי מנתב את **כל** דואר הארגון.
אם Cloudflare Email Routing פעיל על ה-zone הוא מנהל את רשומות ה-MX בעצמו ודורס
הגדרה ידנית.
