...
- **מדיה (MinIO):** ארכיון `tar.gz` **כל 24 שעות**, נשמר
  ...
  **מה זה עולה בדיסק.** ארכיון מלא בכל יום, שנשמר 30 יום, הוא עד 30
  עותקים כמעט זהים: אחסון מדיה של 5GB ⟵ כ-150GB גיבויים. בדקו לפני:
  ...
  אם אין מספיק — הקטינו את `BACKUP_MEDIA_KEEP_DAYS`, או העבירו את
  תיקיית הגיבויים לדיסק נפרד. `BACKUP_MIN_FREE_MB` (ברירת מחדל 2048)
  הוא מרווח הביטחון: מתחתיו ארכיון המדיה **מדולג** עם שגיאה ביומן,
  במקום למלא את הדיסק ולהפיל את המערכת כולה.

  הניקוי רץ **לפני** הכתיבה ולא אחריה — כך המקום לארכיון של היום מגיע
  מזה שפג היום, ואין שיא רגעי של 31 עותקים.
