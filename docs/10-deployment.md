...
‎**למה ההזזה מרוכזת ולא בתוך כל בנייה.** כל יעד נבנה ב-job נפרד
ומסיים בזמן אחר — בשחרור `2c2fd8e` היה הפרש של ארבע דקות וחצי בין
‎`web` לבין `api`. כשכל job הזיז את `:latest` בעצמו, בחלון הזה
‎`:latest` של השירותים הצביע על **שני קומיטים שונים**, ועדכון
שנלחץ בתוכו משך חצי שחרור: מסך הפלטפורמה אז מדווח „השירותים אינם
באותה גרסה”. וכשבניית יעד אחד נכשלה, `:latest` שלו נשאר על הקומיט
הקודם **לצמיתות** — כלומר „הריצו עדכון” לא היה מתקן. `pnpm
verify:release` אוכף את שני התנאים: אף בנייה אינה דוחפת `:latest`,
וההזזה מכסה בדיוק את היעדים שנבנים.
