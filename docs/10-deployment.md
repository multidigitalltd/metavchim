...
בתג הקומיט ⟵ אחרי שכולן נבנו, job אחד מזיז את `:latest` לכולן ⟵
...
‎**למה ההזזה מרוכזת, בתור, ורק לראש `main`.** עד כה כל job הזיז את
‎`:latest` בעצמו כשסיים לבנות, וזה נשבר בפועל: `1811929` ו-`2c2fd8e`
מוזגו בהפרש של 13 שניות, שתי הריצות כתבו לאותם תגים, והריצה הישנה
כתבה את `web:latest` אחרונה. התוצאה ב-Registry הייתה `api`,
‎`workers` ו-`updater` על `2c2fd8e` ו-`web` על `1811929` — **לא
לרגע אלא עד השחרור הבא**. מסך הפלטפורמה דיווח „השירותים אינם באותה
גרסה”, ולחיצה חוזרת על „עדכן” לא תיקנה: כל משיכה הביאה בנאמנות את
אותה תמונה ישנה. נדרשה הרצה ידנית מחדש של ה-workflow.

אותה צורה גם כשבניית יעד נכשלת: השאר כבר הזיזו את `:latest`, ושלו
נשאר על הקומיט הקודם לצמיתות.

‎`pnpm verify:release` אוכף את התנאים: אף בנייה אינה דוחפת
‎`:latest`; ההזזה מכסה בדיוק את היעדים שנבנים; היא בתור ורק לריצה
שהיא ראש `main`; והמצב שנשאר נקרא בחזרה מה-Registry ומפיל את
הבנייה אם השירותים נפרדו.
...
**המסך אומר מה עלה בגורל העדכון, ולא רק שהוא הופעל.** הבקשה חוזרת
מיד (`202`) כי הפעולה נמשכת דקות, ומכאן המסך שואל את הסוכן ב-`GET
/update/status` עד שהוא מדווח סיום — ואז מציג הצלחה או כישלון, עם
המשפט של הסוכן ולא עם ניחוש של המסך.

הסוכן הוא קונטיינר נפרד ששורד את ההחלפה, ולכן הוא זה שמחזיק את
התשובה: ה-API וה-web נהרגים באמצע העדכון ואינם יכולים לזכור דבר.
מאותה סיבה התשובה שורדת גם רענון של הדף — המסך שואל את הסוכן גם
בטעינה, לא רק אחרי לחיצה.

**הכישלון נאמר לפי השלב, כי השלב הוא מה שמכוון את התיקון:**

| שלב | מה קרה | לאן להסתכל |
| --- | --- | --- |
| `pull` | שום שירות לא הוחלף; המערכת שלמה על הגרסה הקודמת | החיבור ל-GHCR וההתחברות אליו |
| `up` | ייתכן שחלק מהשירותים עלו וחלק לא | טבלת הגרסאות באותו מסך, לפני ניסיון נוסף |

**סוכן ישן אומר את זה במפורש.** העדכון אינו מרים את הסוכן, ולכן
מיד אחרי שהשינוי הזה נפרס הסוכן שבשרת עדיין ישן ואינו מכיר את
`/update/status`. במקרה הזה המסך אינו נתקע בהמתנה: הוא מציג את
ההודעה שאומרת שהסוכן ישן ומצטטת את הפקודה שמחליפה אותו. באותה רוח,
סוכן שאינו עונה כלל במשך כמה דקות מפסיק את ההמתנה ואומר לאן להסתכל —
המתנה בלי סוף היא אותה שתיקה בניסוח אחר.

עד לתיקון הזה ההודעה הייתה "העדכון הופעל" בכל מקרה, וזה היה המשפט
האחרון שהמסך אמר: עדכון שנכשל נראה בדיוק כמו עדכון שהצליח, והסיבה
נשארה בלוג של קונטיינר שאיש אינו פותח. `scripts/verify-updater-reports.mjs`
(`pnpm verify:updater`) אוכף שכל פעולה ארוכה בסוכן מדווחת מה עלה
בגורלה — נתיב `GET` שמחזיר את מצבה, ו-`catch` שכותב אליו.
...
**עיקרון שחשוב להבין לפני ההפעלה: Google אינו מקנה גישה למשרד קיים.**
הוא מוכיח בעלות על כתובת אימייל, וזה כל מה שהוא מוכיח. כדי להיכנס
למשרד שכבר קיים המשתמש חייב להיות בו — הוזמן בידי מנהל — אחרת הכניסה
נדחית. בלי הכלל הזה כל בעל חשבון Google היה יכול לייצר לעצמו גישה
לנתונים של משרד אחר.

**כתובת שאין לה חשבון כלל פותחת משרד חדש במסלול החינמי.** זו אינה
חריגה מהכלל למעלה אלא הצד השני שלו: לא ניתנה כאן גישה לשום משרד קיים
— נפתח משרד ריק חדש שהנרשם הוא הבעלים שלו, בדיוק כמו טופס ההרשמה
הציבורי ב-`POST /signup`. למעשה ההוכחה כאן **חזקה יותר**: הטופס שולח
קוד לתיבה ומאמת אותו, ו-`email_verified` של Google הוא אותו אימות
שהספק כבר ביצע. לכן גם אין קוד שני — הוא היה חיכוך בלי רווח.

מה שממשיך לחסום:

| מצב | מה קורה |
|---|---|
| הכתובת קיימת במשרד ופעילה | כניסה רגילה |
| הכתובת קיימת ו**מושבתת** | נדחה — ולא נפתח משרד חדש, אחרת ההשבתה הייתה נעקפת |
| הכתובת אינה קיימת | נפתח משרד חינמי, והנרשם נכנס אליו כבעלים |
| אין מסלול חינמי ציבורי | נדחה בהודעה הרגילה; מסלול אינו מומצא |

המסלול נבחר מתוך המסלולים המוצעים (`isFreePlan`, והראשון לפי
`sortOrder`) ולא לפי קוד צרוב — שינוי שם המסלול במסך הפלטפורמה לא
אמור לשבור את הכניסה בשקט.

**תקרה על פתיחת משרדים, לא על ההתחברות.** נתיב שפותח דייר חדש הוא
הפעולה הרגישה ביותר שנתיב ציבורי יכול לעשות במערכת רב-דיירית, ולכן
שני הנתיבים שפותחים דייר חולקים תקרה אחת:
`MAX_TENANTS_PER_SOURCE` — שלוש פתיחות בשעה לכל מקור בקשה. המונים
נפרדים (טופס ההרשמה ב-`@nestjs/throttler`, והכניסה עם Google ב-Redis)
אבל המספר אחד, כדי ששינוי של אחד לא ישאיר את השני מאחור.

מה שנספר הוא **פתיחות ולא ניסיונות התחברות**: כניסה של סוכן קיים
ודחייה של משתמש מושבת אינן נוגעות במונה כלל. בלי ההבחנה הזו משרד שלם
מאחורי כתובת NAT אחת היה נחסם בשלוש התחברויות.
