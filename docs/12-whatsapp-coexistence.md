...
מדריך ההקמה המקורי (הוסר אחרי שההקמה הושלמה) — כאן הוא מקבל תכנון מלא, בתוספת
...
כללי חלון 24 השעות והתבניות של Meta (ראו [05 §1](05-integrations.md)) חלים כאן
...
1. **אימות עסק** של Multi Digital ב-Business Manager — כבר מכוסה
   — בוצע (Business verification ב-Business Manager).
...
#### מדיה בקו הסוכן — שלושה סוגים, ושער אחד

`audio` (הקלטה), `image` (מודעה מצולמת או תמונת נכס) ו-`document`
(אקסל/CSV לייבוא). שלושתם **חייבים להיות מוכרזים בסכימת ה-zod של
הוובהוק** — zod משמיט מפתח שלא הוכרז, בלי אזהרה, והתוצאה היא פיצ'ר
שנבנה במלואו ומעולם לא רץ. זה קרה בפועל עם `image`, וההגנה עליו
היא `inbound-media-wiring.test.ts`: הוא קורא מה-AST אילו סוגים
הסוכן צורך מהם מדיה — עד קפיצה אחת פנימה למתודה שהענף קורא לה —
ודורש שכל אחד מהם מוכרז **ומועבר**.

קובץ אקסל אינו עובר במנוע ההבנה: אין בו משפט לפרש. הוא מנותב לפני
המנוע, ל-`WhatsappImportService` — שקורא בלבד — ומשם לשלושה צעדים:
„מה יש בקובץ?” (אם הכיתוב לא אמר), תצוגה מקדימה עם מספר השורות
והעמודות שלא זוהו, ואז „אשר”. הכתיבה עצמה היא `ImportWriteService`,
**אותו שירות שנתיב הייבוא מפעיל**, והיכולת נבדקת פעמיים (לפני
הקריאה ולפני הכתיבה) כי הסוכן אינו עובר בבקרים.

#### הגדרות המשרד — מה עובר בשיחה, ומה נשאר במסך

| פעולה | יכולת | מה |
| --- | --- | --- |
| `show_office_settings` | `settings.manage` | שם, מספר רישיון, כתובת, טלפון, דמי תיווך ומועד תשלום (ברירות המחדל להסכמים), ושלושת מתגי הפרסום |
| `update_office_policy` | `settings.manage` | **שלושה מתגים בלבד**: `autoShareProperties`, `autoShareBuyers`, `autoEmailOffers` |

הקריאה דורשת יכולת ולא `null`: דמי התיווך המוסכמים הם מידע מסחרי של
המשרד, לא פרטים של הקורא.

הכתיבה מוגבלת למתגים בכוונה. דמי התיווך, מועד התשלום ומספר הרישיון
הם טקסט חופשי ש**נכנס להסכם חתום**, והכתבה של „שני אחוז פלוס מע״מ”
לשדה שמודפס במסמך משפטי — בלי לראות את המסמך — אינה דבר שצריך לקרות
מטלפון. שם המשרד, הכתובת והטלפון מופיעים בדפי הנחיתה ובהסכמים
ונקבעים פעם אחת.

שני המסלולים כותבים דרך `OfficeSettingsService.update`, שחולץ
מ-`settings.controller.ts` ביום שנוסף לו קורא שני — הסוכן אינו עובר
בבקרים. הוא נושא ארבעה כללים שעותק שני היה מדלג עליהם בשקט:
`lockTenantRow` (‏`settings` הוא מסמך JSON אחד, ושתי כתיבות מקבילות
מוחקות זו את זו בלי שגיאה), מחיקת המפתח במקום שמירת ריק או `false`,
ו-`autoEmailOffersSince` יחד עם `autoEmailOffersCursor` — שתיהן
נמחקות בכיבוי, אחרת הדלקה מחדש מפציצה את כל ההיסטוריה של המשרד
בהצעות.
...
### שני פרטים ב-extras שנכשלים בשקט

**`sessionInfoVersion: "3"` — בשם הזה בדיוק.** זה מה שמבקש את גרסת
ה-session info שממנה מגיעים `waba_id` ו-`phone_number_id`. מפתח בשם
אחר (למשל `version: "v3"`) אינו מוכר ל-Meta, נבלע בלי שגיאה,
והזרימה חוזרת לגרסה ישנה.

**`featureType` — הגדרה, לא קבוע.** `whatsapp_business_app_onboarding`
היא זרימת הדו-קיום, ו-Meta פותחת אותה **רק** לאפליקציה שאושרה
ל-Coexistence. אפליקציה שלא אושרה מקבלת במקום הפופאפ את דיאלוג
ההתחברות הרגיל של פייסבוק ("להמשיך בתור…"), בלי בחירת מספר — כשל
שנראה למתווך כמו מסך מקולקל ולנו כמו "אין שגיאה". לכן הערך מגיע
מהשרת (`whatsappSignupFeatureType` במסך /platform, או
`WHATSAPP_SIGNUP_FEATURE_TYPE`), ומחרוזת ריקה מחזירה Embedded Signup
רגיל — נקודת מילוט בלי גרסה חדשה.

### הקישור שהבוט שולח — שני כשלים שנראים זהים

כל תשובה של הסוכן יכולה לשאת `href`, והוא נשלח כשורת
„👈 ‎`<WEB_ORIGIN><href>`”. שני דברים שונים הופכים אותה ל„העמוד לא
נמצא”, ולמתווך הם נראים אותו דבר:

**‏1. ‎`WEB_ORIGIN` עם לוכסן בסוף.** אז *כל* קישור במערכת הוא
‎`https://host//path` — נתיב שאינו קיים. זה אינו באג בקישור אחד אלא
בכולם בבת אחת, וזו הסיבה שזה נראה כמו „הבוט שבור”. חמישה קוראים
כבר ניקו את הלוכסן בעצמם, כלומר אותו כלל בחמישה עותקים; הנרמול עבר
לסכימת הסביבה (`config/env.ts`), ואיש אינו צריך לזכור.

**‏2. ‎`href` שמצביע לנתיב שלא נבנה.** אלה מחרוזות, ולכן שום טיפוס
אינו יודע אם יש מסך בצד השני — בדיוק כמו `ENTITY_ROUTES` בהתראות.
שתיים נמצאו כך: `/analytics` (מסך הניתוח הוא `/reports`) ו-
‎`/exclusivity` („מעקב בלעדיות” הוא רצועה בראש `/properties`, לא
מסך). השער `verify:notify` בודק עכשיו כל `href` מול עץ הנתיבים של
Next, באותה טענה ששומרת על יעדי ההתראות.

### כשהאירוע לא מגיע — השרת שואל את Meta

אירוע ה-`message` אינו ערוץ מובטח. מתווך שכבר חיבר בעבר מקבל מ-Meta
מסך "קישרת בעבר… רוצה להמשיך עם ההגדרות הקודמות?", ומסלול "המשך"
מדלג על בחירת המספר ולכן אינו משדר `waba_id`/`phone_number_id`;
דפדפן שחוסם `postMessage` בין מקורות עושה את אותו דבר. בשני המקרים
חוזר `code` בלבד.

לכן `POST /whatsapp/connections` מקבל את המזהים כאופציונליים,
ובהיעדרם השרת מזהה בעצמו: `debug_token` ⇒ `granular_scopes` ⇒
`target_ids` (ה-WABA שהטוקן מורשה לנהל) ⇒ `/{waba}/phone_numbers`.
**יותר ממועמד אחד = עצירה, לא ניחוש**: חיבור הקו הלא-נכון מנתב
לקוחות אמיתיים לסוכן הלא-נכון בשקט, ולכן המסך מבקש להתחיל מחדש
ולבחור "עריכת ההגדרות" בחלון של Meta.
...
**שלב 2 — דו-קיום מלא: ✅ נבנה.** `smb_message_echoes` לציר הזמן +
השתקה, ייבוא `history`, `smb_app_state_sync`, `account_update`
וניתוק מסודר, מסך הסטטוס המלא.

> **הערה על העיבוד:** התכנון קרא לתור BullMQ, והמימוש מעבד את
> ההיסטוריה ברקע בתוך תהליך ה-API (`void`, כמו ענף הסוכן) עם
> טרנזקציה פר שיחה. הסיבה: Meta אינה שולחת נתח שוב אחרי 200, כך
> שגם תור לא היה נותן ניסיון חוזר אמיתי — התרופה היחידה היא חיבור
> מחדש. לכן כישלון מסמן את החיבור `error`, והמסך מציע בדיוק את זה
> במקום לאבד נתח בשקט. אם יתברר שנתחים גדולים מכבידים על ה-API,
> המעבר לתור הוא החלפת גוף `handleHistory` בלבד.

> **טרם אומת מול Meta אמיתית.** הקוד כתוב ובדוק מבנית, אבל אף מטען
> `history` אמיתי לא זרם דרכו — האימות הוא שלב 16 במדריך ההקמה.

**שלב 3 — הבוט: ✅ נבנה (הליבה).** `WhatsAppConversation`, תסריט אפיון + מענה על
...
## 10. מודל התמחור — הוכרע

**הגבול עובר במקום שבו עוברת העלות שלנו, לא במקום שנוח לגבות בו:**

| מה | למי משולם | למה |
|---|---|---|
| חיבור המספר, קליטת פניות כלידים, ציר זמן, הדי-אפליקציה, ייבוא היסטוריה | **כלול בכל מסלול** | הודעות נכנסות אינן עולות ל-Meta ולא לנו. גבייה עליהן הייתה גבייה על אפס |
| **בוט המענה ללקוחות** | **תוסף בתשלום** (פיצ'ר `whatsapp_bot`) | כל תשובה היא קריאת LLM שאנחנו משלמים עליה — עלות אמיתית, משתנה, פר-הודעה |
| הודעות יזומות מחוץ לחלון 24h (תבניות) | **המשרד ל-Meta ישירות** | חיוב של Meta ב-WABA של המשרד; אינו עובר דרכנו כלל (§2.2) |

**אכיפה:** `WhatsAppConnectionService.botAllowed()` — שער יחיד בשכבת
השירות ולא ב-Controller, כי הבוט נובע מוובהוק ציבורי שאין בו מסלול
ולא משתמש; `@RequireFeature` לא היה נוגע בו. נעול בבדיקה שגם מוודאת
שקליטת הליד **אינה** מאחורי שער.

### אפליקציה אחת, לא שתיים

"בוט" ו"רק סנכרון" אינם שני מוצרים אצל Meta — הם אותו חיבור בדיוק.
מבחינתה קיימת עובדה אחת: האפליקציה שלנו רשומה ל-Webhooks של ה-WABA.
האם המערכת מנסחת תשובה אוטומטית או שותקת היא החלטה שלנו בשרת.

שתי אפליקציות היו מכפילות את החלק האיטי (App Review + Tech Provider
לכל אחת), ומחייבות מתווך שמשדרג לנתק ולעבור Embedded Signup מחדש —
כולל אובדן ההיסטוריה שסונכרנה. עם השער הזה, הפעלת הבוט היא הדלקת
פיצ'ר במסלול, והמתווך אינו נוגע בכלום.

## 11. הכרעות מסחריות פתוחות (לא חוסמות דבר)

1. **מחיר התוסף** — סכום ומודל (חודשי קבוע כמו `whatsappSeat`, או
   מדורג לפי נפח). התשתית מודדת עלות אמיתית פר-משרד (`AgentEvent`)
   מהיום הראשון, כך שהמספר ייקבע על נתונים ולא על תחושה.
