...
export const AGENT_ACTIONS: readonly AgentActionDef[] = [
  ...
  {
    /*
     * ‎**אותה הזמנה בכתב, בלי לבחור לקוח** — התאום של
     * ‏`open_intake_link`, ומאותו נימוק בדיוק.
     *
     * ‏פעולה נפרדת ולא שדות אופציונליים ב-`send_agreement`: שם שם
     * ‏הלקוח מסומן `alwaysChoose`, כלומר החותם נבחר במפורש תמיד
     * ‏ולעולם אינו „ההתאמה היחידה” — וזה נכון, כי הקישור נושא
     * ‏טוקן שמי שמחזיק בו חותם על מסמך משפטי. לו הפכתי אותו לרשות,
     * ‏שם שנאמר ולא נפתר („תשלח למשה קישור להחתמה”, ובמאגר אין
     * ‏משה) היה נופל בשקט לקישור פתוח במקום לעצור ולשאול.
     *
     * ‎**הסיכון הוא `create` ולא `outbound`**: שום דבר אינו יוצא
     * ‏מהמשרד. נוצר קישור, והמתווך הוא ששולח אותו.
     */
    id: "open_signing_link",
    title: "קישור להחתמה ללקוח חדש",
    when: "קישור חתימה על הזמנה בכתב **בלי לבחור לקוח ובלי נכס** — הלקוח ממלא בעצמו את שמו, מספר הזהות והנכס. בחר בפעולה הזו כשמבקשים „קישור להחתמה” בלי לומר את מי, „קישור כללי להחתמה” או „קישור החתמה ללקוח חדש”. כשנאמר במפורש את מי להחתים יש „קישור לחתימה על הזמנה בכתב”.",
    examples: [
      "תן לי קישור להחתמה",
      "תכין קישור כללי להזמנה בכתב",
      "אני צריך להחתים לקוח חדש שעדיין לא במערכת",
    ],
    capability: "offers.send",
    risk: "create",
    fields: [],
