...
export interface ActivationNudgeInput {
  ...
  planName: string;
  /**
   * ‎**שם המסלול שאליו החשבון באמת יורד — ולא מסלול שקיים בקטלוג.**
   *
   * מועבר **רק** כשההורדה מתבצעת בפועל. ההבחנה הזאת אינה קוסמטית:
   * בגרסה הראשונה הוא נגזר מעצם קיומו של `partnerPlanCode` בהגדרות,
   * ואז ההודעה הבטיחה „רשת שיתופי הפעולה נשארת פתוחה” בזמן ששום דבר
   * בקוד לא שינה את מסלול המשרד — הניסיון פג, `tenantPeriodEnded`
   * החזיר `true`, ו-`AuthGuard` דחה את כל הנתיבים ב-402 (ביקורת
   * Codex). כלומר הבטחה ללקוח על גישה שאין לו.
   *
   * ‎`undefined` = אין הורדה, וההודעה אומרת „החשבון ננעל”.
   */
 ...
 *
 * שני הענפים מתארים **מצב אמיתי**: או שהמשרד הועבר למסלול השותפים,
 * או שהחשבון ננעל. אין ענף שלישי שמתאר משהו שאמור לקרות.
 */
