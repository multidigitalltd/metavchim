...
/**
 * ‎**מה שנאמר בקול ⟵ הסטטוס שהמשרד הגדיר.**
 *
 * ## למה זה לא `enum` בסכימת הפעולה
 *
 * הקטלוג של הסוכן הוא קבוע שנבנה בזמן קומפילציה, ורשימת הסטטוסים
 * שונה לכל משרד. הפיכת הקטלוג לפר-משרד הייתה שינוי בליבת הסוכן
 * ‎(פירוש, פתרון, ביצוע והעובד) בשביל שדה אחד.
 *
 * ‎**וזה גם לא הדפוס הנכון כאן.** `field-spec.ts` כבר קובע ששדות
 * שדורשים ידע שאינו לשוני — תאריך, מיקום — אינם ממולאים בידי המודל
 * אלא נפתרים דטרמיניסטית **אחרי** התשובה. שם המשרד לשלב בתהליך הוא
 * בדיוק אותו סוג נתון: המודל מעביר את מה שנאמר, וההכרעה נעשית כאן,
 * מול הרשימה האמיתית ובקוד שאפשר לבדוק.
 *
 * ## הכרעה, ולא ניחוש
 *
 * 1. התאמה מדויקת אחרי קיפול — „בסבב סיורים” ⟵ „בסבב סיורים”.
 * 2. אחרת, הכלה בכיוון אחד — „סיורים” ⟵ „בסבב סיורים”. מתווך אומר
 *    את החלק המזהה ולא את השם המלא.
 * 3. יותר ממועמד אחד = `null`. „בסיור” שמתאים גם ל„בסבב סיורים” וגם
 *    ל„סיור שני” הוא **אי-ודאות**, וניחוש ביניהן מסמן כרטיס בשלב
 *    שגוי בלי שאיש יבחין. הקורא מדווח את האפשרויות במקום להמציא.
 *
 * ‎**מה שהוסר משימוש אינו ניתן לשיוך מחדש** — הוא מחוץ למועמדים,
 * בדיוק כמו בבורר שבמסך.
 */
export function matchOfficeStatus(
  list: readonly OfficeBuyerStatus[],
  spoken: string,
): OfficeBuyerStatus | null {
  const said = labelKey(spoken);
  if (said === "") return null;
  const open = activeOfficeStatuses(list);

  const exact = open.filter((entry) => labelKey(entry.label) === said);
  if (exact.length === 1) return exact[0]!;
  /* שתי תוויות שקולות בקיפול אינן אמורות להתקיים — `addOfficeStatus`
     דוחה אותן — אבל נתון ישן יכול. אי-ודאות מדווחת ואינה מוכרעת. */
  if (exact.length > 1) return null;

  const partial = open.filter((entry) => {
    const label = labelKey(entry.label);
    return label.includes(said) || said.includes(label);
  });
