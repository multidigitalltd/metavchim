...
export function normalizePhone(raw: string): string {
  ...
  /*
   * מספר לאומי בלי אפס מוביל — ‎583216016‎ במקום ‎0583216016‎.
   *
   * כך אקסל שומר תא שנראה לו כמו מספר, וכך מגיעים מספרים גם
   * מכלי אוטומציה שמעבירים ערך מספרי. בלי ההשלמה הזו אותו אדם
   * מקבל שתי חתימות שונות ונספר כשני לקוחות — או, בייבוא, נדחה
   * לגמרי.
   *
   * ההשלמה מותנית בכך שהתוצאה היא מספר ישראלי תקין. זו אינה
   * ולידציה של הפונקציה (היא נשארת ב-`PhoneSchema`) אלא תנאי
   * לתיקון: בלעדיו כל מחרוזת ספרות זרה הייתה מקבלת ‎+972‎.
   */
  if (/^[2-9]\d{7,8}$/u.test(digits)) return `+972${digits}`;
  return digits;
}
