 ...
 * ‎**שם המוצר — מקום אחד.**
 *
 * ‏הוא כבר היה כתוב בגוף המייל (`renderEmailHtml`), ולכן ההודעה
 * ‏אמרה „מתווכים” בפנים ו-`no_reply` בשורת „מאת”. שני מופעים של
 * ‏אותו שם נפרדים ביום מן הימים; זה כאן, וגוף המייל קורא אותו.
 */
export const PRODUCT_NAME = "מתווכים";

/**
 * ‎**„מאת: no_reply”.**
 *
 * ‏מה שהנמען רואה בעמודת השולח הוא שם התצוגה; בלעדיו לקוחות הדואר
 * ‏נופלים לחלק שלפני ה-‎`@`. ‏`emailFrom` של הפלטפורמה וכתובת
 * ‏התמיכה מאומתות שתיהן כ**כתובת בלבד** (‏`z.string().email()`),
 * ‏כלומר מעולם לא נשאו שם — והתוצאה היא „no_reply” אצל כל מי
 * ‏שקיבל מייל מהמערכת (דיווח המשתמש).
 *
 * ‏שולח שכבר נושא שם אינו נגוע: זה בדיוק המקרה של משרד שחיבר
 * ‏דומיין משלו, ששמו כבר הורכב ב-`formatSender`. סימן `<` הוא
 * ‏ההבחנה, כי זו הצורה היחידה שבה שם יכול להופיע.
 */
export function withSenderName(sender: string, name: string): string {
  return sender.includes("<") ? sender : formatSender(name, sender);
}
