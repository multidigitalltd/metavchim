...
export function extractPropertyFromTranscript(
   ...
   */
  /*
   * מילות השלילה — **מקור אחד לשתי השלילות**.
   *
   * כשכל שוללת נכתבה בנפרד, כל הרחבה תוקנה בצד אחד ונשכחה בשני,
   * ואז השוללת שהוחמצה מאפשרת לחלק החיובי לשמור את ההפך הגמור.
   * הקבוע הזה קיים כדי שלא תהיה „שלילה מעודכנת” ו„שלילה ישנה”.
   *
   * מכוסות: „לא”, „ללא”, „אין”, ו-„אינה/אינו/אינם/איננה/איננו” —
   * כולן גם עם ש' החיבור („דירה **ש**אינה רשומה בטאבו משותף”).
   * כל אלה ניסוחים רגילים לגמרי ולא מקרי קצה.
   *
   * `(?:^|\s)` ולא `\b`: הגבול של `\b` מוגדר על תווי ASCII ואינו
   * עובד בעברית.
   */
  /* הארוכות לפני הקצרות: „אין” הוא רישא של „אינה” ושל „איננה”. */
  const NEG = "(?:^|\\s)ש?(?:ללא|לא|איננ(?:ה|ו|ם)|אינ(?:ה|ו|ם)|אין)\\s+";
   ...
   * (ביקורת Codex).
   */
  const NOT_DIVISIBLE = new RegExp(
    `${NEG}(?:(?:מתאימה|מתאים|ניתנת|ניתן|אפשרות|אפשרית)\\s+)?(?:לחלוקה|מחולקת|מחולק)`,
    "u",
  );
   ...
   * בטאבו משותף”. קבוצת מילות המצב מפורשת ולא `\S+` כללי, כדי
   * ש„לא גדולה אבל בטאבו משותף” לא ייפסל בטעות.
   */
  const NOT_SHARED_TABU = new RegExp(
    `${NEG}(?:(?:רשומה|רשום|רשומים|נמצאת|נמצא|מוגדרת|מוגדר)\\s+)?(?:ב)?טאבו\\s+(?:משותף|שיתופי)`,
    "u",
  );

  /*
   * ‎**„עם חניה” הוא מאפיין; „חניה להשכרה” הוא נכס.**
   *
   * מילת הקישור היא כל ההבדל, ולכן היא זו שנבדקת: `עם`, `כולל`,
   * ‎`+` ושלילה מפורשת מסמנים תכונה של נכס אחר. בלי זה כל דירה עם
   * חניה הייתה נשמרת כחניה.
   */
  const amenityUnless = (word: string): RegExp =>
    new RegExp(
      `(?:עם|כולל|כוללת|ובנוסף|\\+|ללא|בלי|אין|אינה)\\s+[^.,]{0,12}?${word}`,
      "u",
    );
  const AMENITY_PARKING = amenityUnless("חני(?:ה|ות)");
  const AMENITY_STORAGE = amenityUnless("מחסן");
  const AMENITY_BASEMENT = amenityUnless("מרתף");

  /*
   * חלוקה אינה סוג נכס בפני עצמה — היא תכונה של נכס כלשהו.
   * „מגרש לחלוקה”, „חנות מתאימה לחלוקה” ו„קרקע ניתנת לחלוקה” אינם
   * דירות, וסיווגן כ-`divisible_apartment` משנה בשקט את ההתאמה
   * ואת הייצוא (ביקורת Codex).
   *
   * לכן **כל** ניסוחי החלוקה מחייבים הקשר של דירה, ולא רק הקיצור.
   * הנימוק הקודם — שצורת הנקבה „מתאימה/ניתנת” מרמזת על דירה — היה
   * פשוט שגוי: „חנות” ו„קרקע” הן נקבה אף הן. מין דקדוקי אינו
   * מספק הקשר.
   */
  const DIVISIBLE =
  ...
  const typeMap: [RegExp, PropertyFields["propertyType"], RegExp?][] = [
     ...
     * ושמירת „מסחרי” במקומו מאבדת בדיוק את מה שנאמר. „מסחרי”
     * הכללי עצמו יושב בסוף הרשימה.
     */
    ...
    /*
     * ‎**חניה, מחסן ומרתף — אחרי כל סוגי המגורים, ולא לפניהם.**
     *
     * שלושתם סוג נכס **וגם** מאפיין של דירה, ושני התפקידים חיים
     * באותו קובץ: „חניה” ו„מחסן” מופיעים כמה שורות מכאן ב-
     * ‎`featureMap` כ-`hasParking` ו-`hasStorage`. כשהדפוסים ישבו
     * לפני „דירה”, המשפט „דירה עם חניה” נשמר כ**חניה** — ומכיוון
     * שסוג שאינו מבוקש **פוסל** התאמה, הדירה נעלמה מכל קונה
     * מגורים. „דירה ללא חניה” נתפסה באותה דרך (ביקורת Codex).
     *
     * הסדר לבדו אינו מספיק: „וילה עם חניה” אינה תופסת אף דפוס
     * מגורים („וילה” אינה ברשימה), והייתה נופלת לכאן. לכן גם
     * ‎`unless` שדוחה את הניסוח המאפייני — „עם/כולל/ללא/בלי X” —
     * ומשאיר רק את המקרה שבו המילה היא **הנושא**: „מחסן להשכרה
     * ברחוב X”.
     *
     * ‎„דירת מרתף” נתפסת ממילא ב„דירה|דירת” שמעל, וזה הנכון: זו
     * דירה, לא מרתף מסחרי.
     */
    [/חני(?:ה|ות)|חניון/u, "commercial_parking", AMENITY_PARKING],
    [/מחסן|מחסנים/u, "commercial_warehouse", AMENITY_STORAGE],
    [/מרתף/u, "commercial_basement", AMENITY_BASEMENT],
    /*
     * ‎„מסחרי” בלי ענף — אחרון ברשימה, כדי שכל מי שנקב בענף יקבל
     * את הענף. הוא קיים כי „נכס מסחרי בבני ברק” הוא משפט שנאמר,
     * והחלופה לו היא סוג ריק.
     */
    [/מסחרי|מסחרית/u, "commercial"],
  ];
