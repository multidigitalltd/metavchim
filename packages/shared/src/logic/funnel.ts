 ...
 * ‏ובשלוש דרכים שכולן מכוונות: מעבר עצמי למסלול חינמי, העברה
 * ‏לחינמי ממסך הפלטפורמה, והענקת תקופה בתשלום ידנית — שלושתן
 * ‏מוחקות את התאריך **כי אין יותר מה שיפוג**, ואף אחת מהן אינה
 * ‏משאירה כרטיס שיסגור את הרישום כ„שילם”.
 *
 * ‏לעומתן, `PATCH billing-override` מתיר לאפס את התאריך לבדו:
 * ‏המשרד נשאר בניסיון, והתאריך יכול לחזור מאותו מסך ברגע. זה
 * ‏המקרה שבגללו „עוגן חסר” אינו „בלתי אפשרי”.
 *
 * ## ‏הכלל
 *
 * ‏הניסיון עודנו חי כרעיון כל עוד המשרד **בסטטוס ניסיון** ולא
 * ‏קיבל תקופה בתשלום. `paidUntil` נבדק בלי קשר לתאריכו: השאלה
 * ‏אינה „האם ההענקה בתוקף” אלא „האם הייתה הענקה שסיימה את
 * ‏הניסיון” — וזו עובדה שאינה מתבטלת כשההענקה פוקעת.
 */
export function trialAnchorConcluded(tenant: {
  ...
  return tenant.status !== "trial" || tenant.paidUntil !== null;
}
