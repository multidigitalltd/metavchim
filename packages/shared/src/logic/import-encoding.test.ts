...
describe("זיהוי המפריד", () => {
  ...
  /*
   * ‎**ההכרעה היא עקביות, לא ספירה.** בקובץ פסיקים תקין שכותרתו
   * ‏„‎notes; internal; only,city‎” יש שתי נקודות-פסיק ופסיק אחד;
   * ‏ספירה גולמית הייתה בוחרת נקודה-פסיק ומאבדת את עמודת `city`
   * ‏מקובץ שעבד. נקודה-פסיק נותנת 3 תאים בכותרת ו-1 בשורת
   * ‏הנתונים — כלומר אינה מפרידה — ולכן נפסלת (ביקורת Codex).
   */
  it("ומילות כותרת שמכילות מפריד אינן גוברות על עקביות", () => {
    expect(detectDelimiter("notes; internal; only,city\nfoo,Tel Aviv")).toBe(",");
    expect(detectDelimiter("a,b;c;d\nx;y;z")).toBe(";");
  });

  it("ומפריד שאינו עקבי נפסל — תא אחד בכל שורה אינו מפריד", () => {
    // ‏אין מפריד אמיתי בשום מועמד → פסיק, וברירת המחדל היא הנכונה
    expect(detectDelimiter("שורה אחת בלי מפרידים\nעוד שורה")).toBe(",");
  });

  it("ומפריד אחר נבחר רק כשהוא עקבי יותר — שוויון נשאר פסיק", () => {
    expect(detectDelimiter("א,ב\tג")).toBe(",");
  });

  /*
   * ‏ייצוא שמתחיל בשורה ריקה: ספירה על „השורה הראשונה” הייתה
   * ‏מוצאת אפס מפרידים ונופלת לפסיק — כלומר משחזרת בדיוק את
   * ‏התקלה שהפונקציה קיימת כדי למנוע, רק בקובץ אחר.
   */
  it("ושורות ריקות בפתח הקובץ מדולגות", () => {
    expect(detectDelimiter("\r\nשם\tעיר\tמחיר\r\nא\tב\tג")).toBe("\t");
    expect(detectDelimiter("\n\n\nעיר;רחוב;מחיר")).toBe(";");
    // ‏ושורה ריקה אינה הופכת קובץ פסיקים למשהו אחר
    expect(detectDelimiter("\n\nעיר,רחוב,מחיר")).toBe(",");
  });
});
