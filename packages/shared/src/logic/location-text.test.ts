...
/*
 * ‎**המשפחה שנתפסה כאן: `\b` בין אותיות עבריות.**
 *
 * ‎`\w` ב-JavaScript הוא ASCII בלבד, ולכן אות עברית היא „לא-מילה”
 * ‏ו-`\b` לעולם אינו מתקיים בין שתיים מהן. ‎`/עלית\b/u` נראה כמו
 * ‏„עלית כמילה שלמה” ולא התאים לשום דבר — כלל מת מהיום שנכתב, ו-
 * ‎„נצרת עלית” פשוט לא נורמלה ל„נצרת עילית”.
 *
 * ‏שתי הבדיקות האלה הן שני הצדדים: שהכלל **עובד**, ושהוא עדיין
 * ‏**מוגבל למילה שלמה** — כי ביטול הגבול לגמרי היה הופך „מעלית”
 * ‏ל„מעילית”.
 */
describe("כתיב חלופי — גבול מילה עברי", () => {
  it("„עלית” כמילה שלמה נורמלת", () => {
    expect(normalizeLocationName("נצרת עלית")).toBe(normalizeLocationName("נצרת עילית"));
  });

  it("ואינה נוגעת במילה שהיא רק חלק ממנה", () => {
    expect(normalizeLocationName("מעלית")).toBe("מעלית");
    expect(normalizeLocationName("עליתי")).toBe("עליתי");
  });
});
