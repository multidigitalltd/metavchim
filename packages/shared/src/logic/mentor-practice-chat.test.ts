...
describe("איזה תרחיש נאמר במשפט", () => {
  ...
  /*
   * ‏הקדימות נושאת משקל רק כשהמשפט מכיל תווית מלאה
   * ‏של תרחיש אחד ‎**וגם** מילת מפתח של תרחיש שקודם לו
   * ‏ברשימה. התווית לבדה נפתרת נכון גם בלעדיה — ולכן המקרים
   * ‏כאן הם משפטים מורכבים, שבלעדיה היו נפתרים לתרחיש המוטעה.
   */
  it("והתווית קודמת למילות המפתח", () => {
    /* ‏בלי הקדימות: `מוכר.*מחיר` → seller_price */
    expect(
      practiceScenarioFromText(
        "תרגל איתי מוכר שלא רוצה בלעדיות, גם לא על המחיר",
      )?.code,
    ).toBe("seller_exclusive");
    /* ‏בלי הקדימות: `בלעדיות` → seller_exclusive */
    expect(
      practiceScenarioFromText(
        "תרגל איתי קונה שמתלבט על דירה שיש עליה בלעדיות",
      )?.code,
    ).toBe("buyer_hesitant");
    /* ‏בלי הקדימות: `מתלבט` → buyer_hesitant */
    expect(
      practiceScenarioFromText(
        "תרגל איתי קונה עם הצעה נמוכה שעדיין מתלבט",
      )?.code,
    ).toBe("buyer_lowball");
  });

  /*
   * ‏וכל תווית לבדה נפתרת לתרחיש שלה — כדי שמה שהתפריט
   * ‏מציג יהיה משפט שעובד, ולא רק הראשון ברשימה.
   */
  it("וכל תווית בתפריט מגיעה לתרחיש שלה", () => {
    for (const info of PRACTICE_SCENARIO_INFO) {
      expect(
        practiceScenarioFromText(`תרגל איתי ${info.label}`)?.code,
        info.label,
      ).toBe(info.code);
    }
  });
