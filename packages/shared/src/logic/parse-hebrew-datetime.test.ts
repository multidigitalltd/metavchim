...
describe("parseHebrewDateTime", () => {
  ...
  /*
   * וגם **יום בשבוע** חוצץ. הגבול נמדד תחילה לפי ביטויים יחסיים
   * בלבד, ולכן פסוקית שמסתיימת בלוח שנה לא נסגרה: „בעוד שעה
   * לשלוח מסמך שמגיע ביום שני, בעצם ביום שלישי” נתן ליום שלישי
   * לתקן את התזכורת (ביקורת Codex). גם יום בשבוע הוא ביטוי זמן.
   */
  it("וגם ביטוי לוח שנה חוצץ סוגר את הפסוקית", () => {
    const at = parseHebrewDateTime(
      "תזכיר לי בעוד שעה לשלוח מסמך שמגיע ביום שני, בעצם ביום שלישי",
      NOW,
    );
    expect(il(at.date).day).toBe(1);
    expect(il(at.date).hour).toBe(10);
  ...
  /*
   * ו**תאריך מפורש** חוצץ בדיוק כמו „יום שני”.
   *
   * הגבול הכיר רק ביטויי לוח שנה במילים, ולכן „בעוד שעה לשלוח את
   * המסמך שצריך להגיע ב-3 במרץ, בעצם ב-5 במרץ” השאיר את הפסוקית
   * פתוחה: „בעצם” של המסמך נקרא כתיקון של התזכורת, וזו יצאה
   * **ל-3 במרץ** — תאריך ההגעה המקורי, לא אפילו המתוקן. תזכורת של
   * „בעוד שעה” נחתה חודש קדימה.
   */
  it("וגם תאריך מפורש חוצץ סוגר את הפסוקית", () => {
    const at = parseHebrewDateTime(
      "תזכיר לי בעוד שעה לשלוח את המסמך שצריך להגיע ב-3 במרץ, בעצם ב-5 במרץ",
      NOW,
    );
    expect(il(at.date).month).toBe(2);
    expect(il(at.date).day).toBe(1);
    expect(il(at.date).hour).toBe(10);
  });

  /*
   * ו**המוקדם בטקסט** חוצץ, ולא המוקדם בסדר הצורות שבקוד.
   *
   * שלוש צורות התאריך נבדקו בזו אחר זו והראשונה שתפסה ניצחה, ולכן
   * „…שמגיע בתאריך 3.4.2027, בעצם ב-5 במרץ” החזיר דווקא את הצורה
   * השמית — זו שנאמרה **אחרי** „בעצם”. הגבול נמדד אחרי סימן
   * התיקון, התיקון של המסמך נקרא כתיקון של התזכורת, והיא יצאה
   * ל-5 במרץ (ביקורת Codex).
   */
  it("והחוצץ הוא התאריך המוקדם בטקסט, בכל צורה שנאמר", () => {
    const at = parseHebrewDateTime(
      "תזכיר לי בעוד שעה לשלוח מסמך שמגיע בתאריך 3.4.2027, בעצם ב-5 במרץ",
      NOW,
    );
    expect(il(at.date).month).toBe(2);
    expect(il(at.date).day).toBe(1);
  ...
  /*
   * ו**המיקום מכריע בגבול בלבד** — לא בשאלה „איזה תאריך נאמר”.
   *
   * „קבע פגישה בתאריך 3.4.2027, בעצם ב-5 במרץ” הוא תיקון עצמי בלי
   * שום ביטוי יחסי: הדובר החליף את התאריך, והתשובה היא המאוחר.
   * איחוד שתי השאלות לכלל אחד — „המוקדם בטקסט תמיד” — החזיר כאן
   * 3.4 והתעלם מהתיקון (ביקורת Codex). לכן הן נשארו נפרדות.
   */
  it("ותיקון עצמי בין תאריכים מפורשים בוחר את המתוקן", () => {
    const at = parseHebrewDateTime("קבע פגישה בתאריך 3.4.2027, בעצם ב-5 במרץ", NOW);
    expect(il(at.date).month).toBe(3);
    expect(il(at.date).day).toBe(5);
  });

  /*
   * והצד השני של אותו כלל: תיקון שמצביע **ישירות** על תאריך מפורש
   * ממשיך לגבור על ההיסט. הגבול רק מצר את חיפוש הסימן, והיעד
   * שאחריו נקרא בלי חיתוך.
   */
  it("ותיקון אל תאריך מפורש מבטל את ההיסט", () => {
    const at = parseHebrewDateTime("תזכיר לי בעוד שעה, בעצם ב-5 במרץ", NOW);
    expect(il(at.date).month).toBe(3);
    expect(il(at.date).day).toBe(5);
  });
...
describe("תאריך מפורש — מקרי קצה מביקורת Codex", () => {
  ...
  /*
   * ‎**תיקון עצמי בין שני תאריכים מפורשים** (#245).
   *
   * „ב-3 באפריל, בעצם ב-5 במרץ” — הדובר החליף את התאריך, והמערכת
   * קבעה ל-3 באפריל. **התיקון נבלע בשקט**, והפגישה נכנסה ליומן על
   * תאריך תקין למראה שאיש לא ביקש. זה הכישלון המסוכן מבין השניים:
   * שדה ריק המתווך רואה ומשלים, ותאריך שגוי נראה כהחלטה.
   *
   * הסיבה: כל צורה נאספה ב-`exec`, כלומר הופעה אחת לכל צורה. שני
   * תאריכים באותה צורה הגיעו כתאריך אחד, והתיקון לא היה קיים
   * מבחינת הקוד.
   */
  it("תיקון עצמי בין שני תאריכים שמיים בוחר את המתוקן", () => {
    const result = parseHebrewDateTime("קבע פגישה ב-3 באפריל, בעצם ב-5 במרץ", NOW);
    expect(il(result.date)).toMatchObject({ day: 5, month: 3 });
  });

  it("וגם בין שני תאריכים מספריים", () => {
    const result = parseHebrewDateTime(
      "קבע פגישה בתאריך 3.4.2027, בעצם בתאריך 5.3.2027",
      NOW,
    );
    expect(result.date?.getUTCFullYear()).toBe(2027);
    expect(il(result.date)).toMatchObject({ day: 5, month: 3 });
  ...
  /*
   * השורה שעבדה **במקרה**: שתי צורות שונות, ולכן שתיהן נאספו גם
   * כשכל צורה תרמה הופעה אחת. היא נשמרת כדי שהתיקון לא יסיג אותה.
   */
  it("והמקרה המעורב ממשיך לעבוד", () => {
    const result = parseHebrewDateTime("קבע פגישה בתאריך 3.4.2027, בעצם ב-5 במרץ", NOW);
    expect(il(result.date)).toMatchObject({ day: 5, month: 3 });
  });

  /*
   * ‎**והכיוון ההפוך**, שלא עבד גם הוא: כאן הצורה השמית נאמרה
   * ראשונה והמספרית אחרי התיקון, ובחירה לפי סדר הצורות החזירה את
   * השמית — כלומר את מה שהדובר פסל.
   */
  it("תיקון מצורה שמית לצורה מספרית", () => {
    const result = parseHebrewDateTime("קבע פגישה ב-5 במרץ, בעצם בתאריך 3.4.2027", NOW);
    expect(result.date?.getUTCFullYear()).toBe(2027);
    expect(il(result.date)).toMatchObject({ day: 3, month: 4 });
  });

  /*
   * שרשרת מתגלגלת עד הסוף ולא נעצרת על התיקון הראשון — אדם שמתקן
   * את עצמו פעמיים מתכוון לאחרון.
   */
  it("שרשרת תיקונים בוחרת את האחרון", () => {
    const result = parseHebrewDateTime(
      "קבע פגישה ב-3 באפריל, בעצם ב-5 במרץ, סליחה ב-7 במאי",
      NOW,
    );
    expect(il(result.date)).toMatchObject({ day: 7, month: 5 });
  });

  /*
   * ‎**בלי סימן תיקון — הראשון עומד בעינו.** שני תאריכים במשפט אחד
   * אינם בהכרח תיקון („בין 3 באפריל ל-5 במרץ”), והכלל שנוסף כאן הוא
   * מוסף בלבד: הוא זז קדימה רק כשנאמר במפורש שמשהו הוחלף.
   */
  it("שני תאריכים בלי תיקון — הראשון נשאר", () => {
    const result = parseHebrewDateTime("קבע פגישה ב-3 באפריל ואחר כך ב-5 במרץ", NOW);
    expect(il(result.date)).toMatchObject({ day: 3, month: 4 });
  });
});
