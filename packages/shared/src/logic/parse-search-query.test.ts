...
/*
 * ‎**אותה משפחה, במקום השני.** ‎`/\b(ב|ל|מ)\b/u` לא הסיר דבר, ולכן
 * ‏מילית בודדת המשיכה לשארית ומשם לחיפוש הטקסטואלי החופשי — כלומר
 * ‏חיפשו „כהן ב” במקום „כהן”.
 */
describe("מילית בודדת יורדת מהשארית", () => {
  it("„ב” שעומדת לבדה אינה ממשיכה לחיפוש", () => {
    expect(parseSearchQuery("כהן ב תל אביב").rest).toBe("כהן");
  });

  it("ומילה שמתחילה באות הזו אינה נפגעת", () => {
    expect(parseSearchQuery("בוקר").rest).toBe("בוקר");
  });
});
