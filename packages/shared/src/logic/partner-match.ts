 ...
 *
 * ‎**מדוע להסיר את התקציב ולא לחשב תקציב משותף.** תקציב משותף היה
 * ‏עותק שני של רצועת הגמישות, של הרצפה ושל ההבחנה בין מכירה
 * ‏לשכירות — כלל אחד בשני ניסוחים, שהיום מסכימים ובעוד שינוי אחד
 * ‏לא יסכימו. המנוע כבר יודע לדלג על התקציב כשאין תקציב, ולכן
 * ‏„בלי תקציב” נשאל בשפה שלו, והכיסוי שנשאר הוא בדיוק מה שנבדק.
 */
...
export function partnerPairs(
  ...
  for (const candidate of candidates) {
    ...
    /*
     * ‏אותו מנוע, בלי קריטריון התקציב. `undefined` בשני קצות
     * הטווח, כי רצפה בלי תקרה עדיין פוסלת „נמוך מהתקציב”.
     */
    const fit = scoreMatch(
      property,
      { ...req, budgetMinAgorot: undefined, budgetMaxAgorot: undefined },
      weights,
      now,
    );
