...
describe("‏שורת הנכס — רק מה שידוע", () => {
   ...
   * ‏(ביקורת Codex). מה שאין בכותרת — שטח ומחיר — נשאר.
   */
  it("‏אינה חוזרת על מה שכבר בכותרת", () => {
  ...
  /*
   * ‎**והמיקום נבדק לפי רכיביו.** „קריית שרת, חולון” כמחרוזת אחת
   * ‏אינו מוכל בכותרת, ולכן העיר חזרה בכל זאת (ביקורת Codex).
   */
  it("‏השכונה נשארת והעיר שבכותרת יורדת", () => {
    const line = pitchPropertyLine({
      title: "דירת 4 חדרים בחולון",
  ...
  /* ‏ושכונה ועיר שאינן בכותרת נשארות שתיהן, כמו שהיה */
  it("‏מיקום שאינו בכותרת נשאר שלם", () => {
    const line = pitchPropertyLine({
      title: "פנינה מול הים",
      neighborhood: "לב הפארק",
      city: "רעננה",
    ...
    expect(line).toBe("פנינה מול הים — לב הפארק, רעננה");
  });

  /* ‏וגם כותרת שיווקית שכתב המשרד — אותו כלל בדיוק */
  it("‏גם כשהכותרת שיווקית ומזכירה את העיר", () => {
    const line = pitchPropertyLine({
    ...
    expect(line).toBe("דירה מהממת בחולון — 3 חדרים");
  });
