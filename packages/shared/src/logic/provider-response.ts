 ...
 *
 * ‎`key` נמצא כאן כי **התיעוד של 015 מפנה אליו במפורש**: על שגיאה
 * הוא כותב „Check the key and message fields for more details”.
 * כלומר זהו קוד השגיאה שאומר איזה פרמטר נדחה — בדיוק המידע שחסר
 * כדי להפסיק לנחש מול 404 שמשמעותו „לא קיים **או** אין הרשאה”.
 *
 * בלעדיו הוא נפל לענף האורך והוצג כ-`responses.key(15 תווים)`:
 * הודעה שאומרת שיש תשובה ואינה אומרת מה בה.
 *
 * הוא עובר את אותו ניקוי כמו כל ערך אחר — מחיקת כתובות, מחיקת
 * הסודות, וגבול אורך — ולכן הוספתו אינה פותחת ערוץ לטקסט חופשי.
 */
const SAFE_VALUE_KEYS = ["status", "code", "result", "format", "filetype", "type", "key"];
