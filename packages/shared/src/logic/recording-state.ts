 ...
 * דקות. הסבב רץ כל חמש דקות ובחר כל שיחה שטרם נוסתה **בלי שום תנאי
 * על גילה** — כלומר שיחה בת חצי דקה נמשכה בטיק הבא, לפני שהיה מה
 * למשוך. 015 סירבה, וזה נראה למשרד כתקלה (דיווח מהשטח).
 *
 * שלוש תוצאות יצאו מזה, והשלישית היא החמורה:
 *
 * 1. המסך הכריז „המשיכה נכשלה” על שיחה בת דקה — שאינה תקלה כלל.
 * 2. הכישלון כתב חותמת ניסיון, ולכן ההקלטה חיכתה ל`RETRY_AFTER_MS`
 *    — חצי שעה — במקום להגיע בעוד שתי דקות.
 * 3. הסירוב נספר ברצף שעוצר את הסבב, ומשרד שסיים שלוש שיחות ברצף
 *    הקפיא את התור של **כל** המשרדים.
 *
 * ‎**זה אינו עיכוב שמישהו ממתין לו.** ההקלטה מופיעה דקות אחרי
 * השיחה בכל מקרה, והמסך אומר „בדרך מהמרכזייה” בדיוק כפי שצריך —
 * כי בלי ניסיון אין שגיאה, ולכן אין מה להציג כתקלה.
 */
export const RECORDING_FIRST_ATTEMPT_GRACE_MS = 5 * 60 * 1000;

/**
 * ‎**שיחה „צעירה” — כזו שעדיין סביר שההקלטה שלה בדרך.**
 *
 * הגבול בין שתי משפחות של כישלון. בתוך השעה הראשונה הסיבה הסבירה
 * היא „טרם הוכן”, שנפתרת מעצמה; אחריה הסיבות הן מנוי שפג, הקלטה
 * שנמחקה או תקלה של ממש — ואלה אינן נפתרות בדקות.
 */
export const RECORDING_YOUNG_CALL_MS = 60 * 60 * 1000;

/**
 * ניסיון חוזר על שיחה צעירה.
 *
 * ‎`RETRY_AFTER_MS` הוא חצי שעה, וזה נכון ל„מנוי פג” — אבל אכזרי
 * ל„עוד לא מוכן”: הקלטה שהייתה מוכנה בעוד שתי דקות הגיעה אחרי
 * שלושים. קצב הסבב הוא הגבול התחתון הטבעי כאן — ניסיון תכוף יותר
 * מהסבב עצמו אינו אפשרי ממילא.
 */
