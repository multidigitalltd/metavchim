...
/**
 * ‎**התווית שהנכס נושא ברשת** (ביקורת Codex, P1).
 *
 * ‏רישום משותף אינו מאפיין כמו מעלית: הוא משנה את כל אופן העסקה —
 * ‏אין חלקה נפרדת, נדרשת הסכמת שותפים, והמימון מורכב. סוכן ממשרד
 * ‏אחר שמבקש חיבור על מודעה כזו עושה צעד שקשה לחזור ממנו, ולכן
 * ‏הוא חייב לדעת לפניו.
 *
 * ‏קבוע אחד לכל הכרטיסים — הצ׳יפ, שורת הפירוט וההצהרה — כדי
 * ‏שהנוסח לא יתפצל בין המסכים.
 */
export const SHARED_TABU_NETWORK_LABEL = "רישום משותף (מושאע)";
