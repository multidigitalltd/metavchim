...
/**
 * ‎**האם השורה הזו היא ראיה משפטית שנשמרת — ולכן שייכת לארכיון.**
 *
 * זהו בדיוק מה שמחיקת לקוח משמרת: הצהרה על הסכם חתום, עם תאריך
 * חתימה. כל השאר — תעודת זהות, אישור זכויות, נספח — הוא מסמך של
 * הלקוח עצמו, וזכות המחיקה חלה עליו במלואה; הוא **נמחק**.
 *
 * ‎**כאן, ולא בכל שער בנפרד.** הרשימה והשער חייבים להסכים: רשימה
 * שסוננה ושער שלא סונן פירושם שמזהה ידוע פותח בדיוק את מה שהרשימה
 * הסתירה. זה קרה כאן פעמיים — פעם על מבחן היתמות ופעם על מבחן
 * השימור (ביקורת Codex), ולכן ההכרעה יושבת במקום אחד.
 */
export function isRetainedDocument(kind: string, signedOn: Date | string | null): boolean {
  return signedOn !== null && documentUnlocksOffers(kind);
}
