...
export const TELEPHONY_PROVIDERS: readonly TelephonyProvider[] = [
  ...
  },
  {
     ...
     *
     * הקו שמצלצל ראשון הוא **של הסוכן ולא של המשרד**, כי זו כל
     * הנקודה של חיוג בלחיצה; קו משרדי אחד לכולם היה מחבר את הלקוח
     * למי שבמקרה הרים. הסדר: שלוחת הסופטפון שלו, ואם אין — הנייד
     * מהפרופיל, ורק בסוף `defaultLine`.
     */
    id: "015",
    label: "015",
    fields: [
      /*
       * ‎`customer` — **מספר הלקוח של המשרד ב-015.**
       *
       * הוא נדרש ב-`calls/make` (כך מסרה התמיכה של 015), ולא נשלח
      ...
      {
        key: "defaultLine",
        label: "קו ברירת מחדל (כשלסוכן אין שלוחה ואין טלפון בפרופיל)",
        secret: false,
      },
...
export function isGeneratedCallSummary(summary: string | null | undefined): boolean {
  ...
  if (text === "") return true;
  return /^שיחה (נכנסת|יוצאת)( שלא נענתה| ללא מענה| · .+)?$/u.test(text);
}
 ...
 *
 * **השיחה דו-שלבית**: 015 מצלצל קודם ל-`snumber` (הקו של הסוכן),
 * וכשהוא עונה — מחייג ל-`cnumber` (הלקוח) ומחבר ביניהם. זה המודל
 * הנכון לחיוג בלחיצה: הסוכן לא צריך להקליד, והלקוח רואה שיחה
 * מהמשרד.
 *
 * ## הצורה כאן היא זו שהתמיכה של 015 מסרה
 *
 * מה שהיה כאן קודם נבנה מקריאה בתיעוד, וחלקו היה השערה. שלושה
 * מהערכים היו פשוט שגויים:
 *
 * - ‎`customer` **לא נשלח כלל.** הוא נדרש, ובלעדיו הבקשה אינה
 *   מזוהה מול חשבון המשרד.
 * - ‎`ctype` נשלח **ריק**, מתוך נימוק מפורש בקוד ש„ריק = מספר
 *   חיצוני”. הערך הנכון הוא `forward`. הנימוק הישן היה הגיוני
 *   ולא נבדק מול הספק — וזה בדיוק סוג ההימור שנראה כמו החלטה.
 * - מזהה המתקשר ישב על הרגל **השנייה** בלבד.
 *
 * ## למה `callerid1` הוא מספר הלקוח
 *
 * הרגל הראשונה היא זו שמצלצלת אצל הסוכן. מזהה המתקשר עליה הוא מה
 * ש**הסוכן** רואה, ולכן הוא מספר הלקוח: לוחצים „חייג” בכרטיס,
 * הסופטפון מתעורר, ועל המסך כתוב למי מתחברים. בלי זה השיחה קופצת
 * בלי הקשר, וזה הרגע שבו עונים „הלו?” במקום בשם המשרד.
 *
 * ‎`answer1=1` משלים אותה: הרגל של הסוכן **נענית מעצמה**. לחיצה
 * אחת ואתה בשיחה, בלי צלצול ובלי הרמה — וזו כל הנקודה של סופטפון.
 *
 * ## מה שאינו נשלח, ולמה
 *
 * ‎`wait` היה כאן כדי לקבל `callid` בתשובה. הוא אינו בתבנית של
 * התמיכה, והוסר. המחיר מצומצם ממה שההערה הישנה טענה: המזהה נכתב
 * ליומן הביקורת בלבד, ורישום השיחה עצמה נבנה מה-Webhook לפי
 * ה-`providerCallId` שהמרכזייה שולחת — כלומר הקישור לכרטיס אינו
 * תלוי בו.
 *
 * הפונקציה מחזירה URL ולא שולחת אותו: כך היא ניתנת לבדיקה בלי רשת,
 * וכל ההחלטות על שמות הפרמטרים נבדקות במקום אחד.
 */
export const PBX015_MAKE_URL = "https://www.015pbx.net/local/api/json/calls/make/";

export function build015DialUrl(input: {
  /** מספר הלקוח של המשרד ב-015. */
  customer: string;
  authUsername: string;
  authPassword: string;
  /** הקו של הסוכן — שלוחת SIP, או הנייד שלו כשאין שלוחה. */
  agentLine: string;
  /**
   * האם `agentLine` היא שלוחת סופטפון.
   *
   * זה מה שקובע `answer1`: שלוחה יכולה להיענות מעצמה, ונייד לא —
   * ‎„ענה בשבילי” על טלפון סלולרי הוא הוראה שאין לה משמעות, ובמקרה
   * הרע היא שיחה שיוצאת לאוויר בלי שאיש מחזיק את המכשיר.
   */
  softphone: boolean;
  /** הלקוח — הצד שאליו מחברים אחרי שהסוכן ענה. */
  destination: string;
  /** מזהה המתקשר שיוצג ללקוח, כשהמשרד הגדיר אותו. */
  callerId?: string;
}): string {
  /*
  ...
  const params: [string, string][] = [
    ...
    // "phone" = קו במרכזייה — שלוחה או מספר שמוגדר בה, ולא יעד חיצוני
    ["stype", "phone"],
    ["snumber", input.agentLine],
    // "forward" — היעד הוא מספר חיצוני שמחייגים אליו ומעבירים אליו
    ["ctype", "forward"],
    ["cnumber", input.destination],
    // מזהה המתקשר על הרגל **הראשונה** — זה מה שהסוכן רואה
    ["callerid1", input.destination],
    ...(input.softphone ? ([["answer1", "1"]] as [string, string][]) : []),
    /*
     * הרגל השנייה — מה שהלקוח רואה. אינה בתבנית של התמיכה, ונשלחת
     * רק כשהמשרד הגדיר מזהה מתקשר: היא תוספת ולא סתירה, והשמטתה
     * הייתה מבטלת בשקט שדה שמשרדים כבר מילאו.
     */
    ...(input.callerId ? ([["callerid2", input.callerId]] as [string, string][]) : []),
  ];
  /*
   * ‎`encodeURIComponent` מקודדת `;` ל-`%3B` — הוא תו שמור ואינו
   * נמצא ברשימת החריגים שלה (`-_.!~*'()`). זה מה שמאפשר להשתמש בו
   * כמפריד: סיסמה שיש בה נקודה-פסיק אינה נחתכת לשני פרמטרים.
   * הבדיקה נועלת את התכונה הזו במקום להסתמך על הזיכרון שלנו לגביה.
   */
  const query = params.map(([key, value]) => `${key}=${encodeURIComponent(value)}`).join(";");
  /*
   * נקודה-פסיק בסוף, כמו בתבנית. הפרמטר האחרון משתנה לפי מה
   * שהוגדר, ומפריד עוקב הוא הצורה שנמסרה — לא ניסוח שלנו.
   */
