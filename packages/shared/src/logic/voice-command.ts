...
const RULES: {
   ...
   */
  /*
   * ‎**„המר ללקוח” — והענף השלישי אינו קישוט.**
   *
   * ‏„מי שהתקשר” נתפס אחרת לגמרי: ל-`add_lead` יש כלל בביטחון נמוך
   * ‏על „התקשר”, וכל משפט שמזכיר אותו ואינו נתפס **כאן** נופל לשם
   * ‏— כלומר „תוסיף את מי שהתקשר כלקוח” היה פותח ליד ריק במקום
   * ‏לשאול מה הצד השני. אימות: בלי הכלל הזה כל הניסוחים חוץ
   * ‏מזה מחזירים `unknown`, וזה מחזיר `add_lead`.
   *
   * ‏שלושת פעלי ההמרה נכתבים במפורש ולא כ-`ה?מר`: „תמיר את השיחה
   * ‏ללקוח” ו„המר את השיחה ללקוח” — שני הניסוחים הטבעיים ביותר
   * ‏אחרי לחיצה על הכפתור — לא נתפסו בגרסה הקודמת.
   */
  {
    action: "convert_call",
    pattern: /(?:המר|תמיר|להמיר|הפוך|תהפוך|להפוך)\s+(?:\S+\s+){0,3}ל?לקוח|תפתח\s+לקוח\s+מ?ה?שיחה|מי\s+שהתקשר\s+(?:\S+\s+){0,2}[כל]לקוח/u,
    confidence: "high",
  },
  {
