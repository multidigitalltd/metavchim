...
const ENTITY_ROUTES: Record<string, (id?: string) => string> = {
  ...
  /*
   * ‎**הישג שבועי — אין לו מסך משלו, ולא צריך.**
   *
   * ‏שתי ההתראות שנושאות אותו מובילות לאותו מקום מסיבות הפוכות:
   * המנהל מגיע לכרטיס „הצוות שלך סגר את השבוע” כדי להגיב, והסוכן
   * מגיע למסך שלו כדי לראות מה נכתב לו. אותו נתיב, אותו הקשר —
   * כמו `offer` שמוביל לרשימה ולא לפריט.
   */
  mentor_achievement: () => "/mentor",
  /*
