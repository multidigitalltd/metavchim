...
export const WHATSAPP_TEMPLATE_PARAMS = {
  ...
  intake: ["form_link"],
  /*
   * ‎**תזכורת לפני סיור — שדות, ולא נוסח שלם.**
   *
   * הגרסה הראשונה העבירה משתנה אחד: כל טקסט התזכורת שהמשרד ניסח.
   * שתי בעיות, ושתיהן אצל Meta ולא אצלנו:
   *
   ...
   *
   * המחיר: הנוסח שהמשרד ניסח נשלח **במייל** כלשונו, ובוואטסאפ
   * יוצאים השדות עצמם. עדיף על תבנית שנדחית או מסווגת כשיווקית,
   * שאז לא יוצא דבר.
   */
  viewingReminder: [
    "customer_name",
